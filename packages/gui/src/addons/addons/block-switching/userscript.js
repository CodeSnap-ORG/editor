        ...
        try {
            ...
            if (parentBlock) {
                ...
                parentConnection = parentConnections.find(
                    (c) =>
                        c.targetConnection &&
                        c.targetConnection.sourceBlock_ === block,
                );
                ...
                const blockToParentConnection = blockConnections.find(
                    (c) =>
                        c.targetConnection &&
                        c.targetConnection.sourceBlock_ === parentBlock,
                );
            ...
            for (const child of Array.from(xml.children)) {
                ...
                const newName =
                    opcodeData.remapInputName &&
                    opcodeData.remapInputName[oldName];
                if (newName) {
                ...
                if (newShadowType) {
                    ...
                    fieldNode.setAttribute(
                        "name",
                        getShadowFieldName(newShadowType),
                    );
                }
            ...
            if (opcodeData.createInputs) {
                for (const [inputName, inputData] of Object.entries(
                    ...
                    shadowFieldElement.innerText = callIfFunction(
                        inputData.value,
                    );
            ...
            if (parentConnection) {
                ...
                const newBlockConnection = newBlockConnections.find(
                    (c) => c.type === blockConnectionType,
                );
        ...
        (items, block) => {
            if (!addon.self.disabled) {
                ...
                const customArgsMode = addon.settings.get("customargs")
                    ? addon.settings.get("customargsmode")
                    : "off";
                if (
                    customArgsMode !== "off" &&
                    [
                    ...
                    if (customArgsMode === "all") {
                        ...
                        const customBlockObj =
                            customBlocks[
                                root.getChildren(true)[0].getProcCode()
                            ];
                        switch (type) {
                            case "argument_reporter_string_number":
                ...
                if (
                    ...
                    const delBlockIndex = items.findIndex(
                        (item) => item.text === ScratchBlocks.Msg.DELETE_BLOCK,
                    );
