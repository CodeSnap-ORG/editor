export default async ({ addon, console }) => {
    ...
    vm.stopDrag = function (...args) {
        const allowDrag =
            shiftKeyPressed ||
            (addon.settings.get("drag_while_stopped") &&
                !addon.tab.redux.state.scratchGui.vmStatus.running);
        if (allowDrag || addon.self.disabled)
            return oldStopDrag.call(this, ...args);
