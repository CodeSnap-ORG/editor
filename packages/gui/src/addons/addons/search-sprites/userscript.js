export default async function ({ addon, console, msg }) {
    ...
    const search = (query) => {
        ...
        for (const sprite of spritesContainer.children) {
            const visible =
                ...
                (containsQuery(
                    sprite.children[0].children[2].children[0].innerText,
                ) &&
    ...
    while (true) {
        await addon.tab.waitForElement(
            "div[class^='sprite-selector_items-wrapper']",
            {
                markAsSeen: true,
                reduxEvents: [
        ...
        spritesContainer = document.querySelector(
            '[class^="sprite-selector_items-wrapper"]',
        );
        spriteSelectorContainer = document.querySelector(
