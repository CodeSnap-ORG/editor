import {
    normalizeHex,
    getHexRegex,
} from "../../libraries/common/cs/normalize-color.js";
...
export default async ({ addon, console, msg }) => {
        ...
        const onEyeDropperClosed = ({ detail }) => {
            ...
            setTimeout(() => {
                document.body.classList.remove(
                    "sa-hide-eye-dropper-background",
                );
            }, 50);
        };
    ...
    const addColorPicker = (editor) => {
        ...
        const saColorPickerColor = Object.assign(
        ...
        const saColorPickerText = Object.assign(
            document.createElement("input"),
            {
        ...
        saColorPickerColor.addEventListener("input", () =>
            rateLimiter.limit(() =>
                setColor(
                    (saColorPickerText.value = saColorPickerColor.value),
                    element,
                ),
            ),
        );
    ...
    const originalShowEditor =
        ScratchBlocks.FieldColourSlider.prototype.showEditor_;
    ScratchBlocks.FieldColourSlider.prototype.showEditor_ = function (...args) {
        const r = originalShowEditor.call(this, ...args);
        addColorPicker(this);
        return r;
    };
    const originalCallbackFactory =
        ScratchBlocks.FieldColourSlider.prototype.sliderCallbackFactory_;
    ScratchBlocks.FieldColourSlider.prototype.sliderCallbackFactory_ =
        function (...args) {
            ...
            return (event) => {
                ...
                if (div) {
                    const saColorPickerColor = div.querySelector(
                        ".sa-color-picker-color.sa-color-picker-code-color",
                    );
                    const saColorPickerText = div.querySelector(
                        ".sa-color-picker-text.sa-color-picker-code-text",
                    );
