export default async function ({ addon, console }) {
    ...
    const updateStyles = () => {
        previewInner.classList.toggle(
            "sa-comment-preview-delay",
            addon.settings.get("delay") !== "none",
        );
        previewInner.classList.toggle(
            "sa-comment-preview-reduce-transparency",
            addon.settings.get("reduce-transparency"),
        );
        previewInner.classList.toggle(
            "sa-comment-preview-fade",
            !addon.settings.get("reduce-animation"),
        );
    };
    ...
    const getBlock = (id) =>
        ...
        vm.runtime.flyoutBlocks.getBlock(id);
    const getComment = (block) =>
        block && block.comment && vm.editingTarget.comments[block.comment];
    const getProcedureDefinitionBlock = (procCode) => {
        const procedurePrototype = Object.values(
            vm.editingTarget.blocks._blocks,
        ).find(
            (i) =>
                i.opcode === "procedures_prototype" &&
                i.mutation.proccode === procCode,
        );
    ...
    document.addEventListener("mouseover", (e) => {
        ...
        const el = e.target.closest(
            ".blocklyBubbleCanvas > g, .blocklyBlockCanvas .blocklyDraggable[data-id]",
        );
        ...
        if (
            ...
            e.target.closest(".blocklyBlockCanvas .blocklyDraggable[data-id]")
        ) {
            ...
            if (addon.settings.get("hover-view-block") && comment) {
                ...
                const procedureDefinitionBlock =
                    getProcedureDefinitionBlock(procCode);
    ...
    document.addEventListener("mousemove", (e) => {
        ...
        if (
            addon.settings.get("follow-mouse") &&
            !previewInner.classList.contains("sa-comment-preview-hidden")
        ) {
            updateMousePosition();
        }
    });
