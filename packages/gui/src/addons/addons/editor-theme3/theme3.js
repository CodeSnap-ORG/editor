...
export default async function ({ addon, console, msg }) {
    ...
    const isColoredTextMode = () =>
        textMode() === "colorOnWhite" || textMode() === "colorOnBlack";
    ...
    const secondaryColor = (primary) => {
        if (isColoredTextMode())
            return alphaBlend(
                ...
                multiply(primary, { a: 0.15 }),
            );
        if (textMode() === "black")
            return brighten(primary, { r: 0.6, g: 0.6, b: 0.6 });
    ...
    const tertiaryColor = (primary) => {
        ...
        if (textMode() === "black")
            return multiply(primary, { r: 0.65, g: 0.65, b: 0.65 });
    ...
    const quaternaryColor = (primary) => {
        if (isColoredTextMode())
            return alphaBlend(
                primaryColor(primary),
                multiply(primary, { a: 0.25 }),
            );
        if (textMode() === "black")
            return brighten(primaryColor(primary), { r: 0.4, g: 0.4, b: 0.4 });
    ...
    const fieldBackground = (object) => {
        ...
        if (
            object instanceof Blockly.Block ||
            object instanceof Blockly.Field
        ) {
            const block =
                object instanceof Blockly.Block ? object : object.sourceBlock_;
            if (isColoredTextMode() || textMode() === "black") {
                ...
                if (isColoredTextMode())
                    return alphaBlend(
                        actualPrimary,
                        multiply(block.getColourTertiary(), { a: 0.25 }),
                    );
    ...
    const uncoloredTextColor = () => {
        if (textMode() === "white" || textMode() === "colorOnBlack")
            return "#ffffff";
    ...
    const categoryIconBackground = (primary) =>
        isColoredTextMode() ? quaternaryColor(primary) : primaryColor(primary);
    const categoryIconBorder = (primary) => tertiaryColor(primary);

    const useBlackIcons = () =>
        textMode() === "black" || textMode() === "colorOnWhite";
    const iconPath = () =>
        `/icons/${useBlackIcons() ? "black_text" : "white_text"}`;
    ...
    Blockly.Toolbox.Category.prototype.createDom = function () {
        ...
        if (
            !addon.self.disabled &&
            ["a-b", "videoSensing", "text2speech"].includes(this.id_)
        ) {
            const match = this.iconURI_.match(
            ...
            if (match) {
                ...
                const category =
                    this.id_ === "a-b" ? saCategory : extensionsCategory;
        ...
        if (!addon.self.disabled && !this.iconURI_) {
            const category = categories.find(
                (item) => item.categoryId === this.id_,
            );
            if (category) {
                const primary = addon.settings.get(category.settingId);
                this.bubble_.style.backgroundColor =
                    categoryIconBackground(primary);
        ...
        if (!addon.self.disabled && isColoredTextMode()) {
            for (const input of this.inputList) {
                if (input.outlinePath) {
                    const sourceBlock =
                        this.isShadow() && this.getParent()
                            ? this.getParent()
                            : this;
                    const category = categories.find(
                        (i) => i.categoryId === sourceBlock.category_,
                    );
                    if (category) {
                        input.outlinePath.setAttribute(
                            "fill",
                            fieldBackground(
                                addon.settings.get(category.settingId),
                            ),
                        );
                    }
                }
    ...
    Blockly.BlockSvg.prototype.showContextMenu_ = function (e) {
        ...
        if (!addon.self.disabled) {
            Blockly.WidgetDiv.DIV.style.setProperty(
                "--editorTheme3-hoveredItem",
    ...
    const oldFieldVariableGetterInit =
        Blockly.FieldVariableGetter.prototype.init;
    ...
    const oldFieldDropdownShowEditor =
    ...
    Blockly.FieldDropdown.prototype.showEditor_ = function () {
        ...
        if (!addon.self.disabled) {
            ...
            // Active hover color
            Blockly.DropDownDiv.DIV_.style.backgroundColor =
                removeAlpha(primaryColor);
            if (isColoredTextMode()) {
                Blockly.DropDownDiv.getContentDiv().style.setProperty(
                    "--editorTheme3-hoveredItem",
                    fieldBackground(this),
                );
            } else {
                Blockly.DropDownDiv.getContentDiv().style.removeProperty(
                    "--editorTheme3-hoveredItem",
                );
            }
        }
    ...
    const oldFieldVerticalSeparatorInit =
        Blockly.FieldVerticalSeparator.prototype.init;
    Blockly.FieldVerticalSeparator.prototype.init = function () {
        ...
        if (
            !addon.self.disabled &&
            (isColoredTextMode() || textMode() === "black")
        ) {
            this.lineElement_.setAttribute(
                "stroke",
    ...
    const apply = () => {
        ...
        if (textMode() === "colorOnWhite")
            blockColors.fieldShadow = "rgba(0, 0, 0, 0.15)";

        const extensions = {
            music: {
                blockIconURI: addon.self.getResource(
                    `${iconPath()}/extensions/music.svg`,
                ),
            },
            pen: {
                blockIconURI: addon.self.getResource(
                    `${iconPath()}/extensions/pen.svg`,
                ),
            },
            text2speech: {
                blockIconURI: addon.self.getResource(
                    `${iconPath()}/extensions/text2speech.svg`,
                ),
            },
            translate: {
                blockIconURI: addon.self.getResource(
                    `${iconPath()}/extensions/translate.${useBlackIcons() ? "svg" : "png"}`,
                ),
            },
            videoSensing: {
                blockIconURI: addon.self.getResource(
                    `${iconPath()}/extensions/videoSensing.svg`,
                ),
            },
        };
        ...
        const newTheme = addon.tab.redux.state.scratchGui.theme.theme.set(
            "blocks",
            BLOCKS_CUSTOM,
        );
