export default async function ({ addon }) {
    ...
    const isSupportedElement = (el) => {
        if (el.classList.contains("blocklyHtmlInput")) return true;
            ...
            if (
                el.matches(
                    "[class*=sprite-info_sprite-info_] [class*=input_input-small_]",
                )
            ) {
                ...
                el.matches(
                    "[class*=paint-editor_editor-container-top_] input[type=number]",
                )
            ) {
    ...
    document.body.addEventListener("keydown", (e) => {
        ...
        if (addon.settings.get("useCustom")) {
            ...
            if (Number.isNaN(valueAsFloat)) {
                ...
                valueAsFloat === 0 ||
                (valueAsFloat < 100000000 && valueAsFloat > 0.00000099)
            ) {
        ...
        const decimalCount = Math.max(
            amountOfDecimals(e.target.value),
            amountOfDecimals(changeBy.toString()),
        );
        ...
        const newValue = shiftDecimalPointToLeft(
            newValueAsBigInt.toString(),
            decimalCount,
        );

        if (e.target.className.includes("input_input-form_")) {
            Object.getOwnPropertyDescriptor(
                e.target.constructor.prototype,
                "value",
            ).set.call(e.target, newValue.toString());
            ...
            if (!inputMap.has(e.target))
                inputMap.set(e.target, { time: null, steps: -1 });
