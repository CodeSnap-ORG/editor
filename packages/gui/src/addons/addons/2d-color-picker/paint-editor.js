...
export default async ({ addon, console, msg }) => {
    ...
    const getColor = (element) => {
        ...
        const color =
            state.scratchPaint.color[`${fillOrStroke}Color`][
                primaryOrSecondary
            ];
        if (color === null || color === "scratch-paint/style-path/mixed")
            return;
    ...
    const setColor = (hex, element) => {
        ...
        if (!addon.tab.redux.state || !addon.tab.redux.state.scratchPaint)
            return;
        ...
        const onEyeDropperOpened = ({ detail }) => {
            ...
            setTimeout(() => {
                const previousTool =
                    addon.tab.redux.state.scratchPaint.color.eyeDropper
                        .previousTool;
    ...
    while (true) {
        ...
        const element = await addon.tab.waitForElement(
        ...
        function updateColor() {
            rateLimiter.limit(() => {
                ...
                saColorPicker.style.background =
                    "#" + convertToGeneralColor(getColor(element));
            });
        }
        ...
        saColorPicker.style.background =
            "#" + convertToGeneralColor(defaultColor || "ff0000");

        const saColorPickerImage = Object.assign(
            document.createElement("img"),
            {
                className: "sa-2dcolor-picker-image",
                src: addon.self.getResource(
                    "/assets/sv-gr.png",
                ) /* rewritten by pull.js */,
                draggable: false,
            },
        );
        const saColorPickerHandle = Object.assign(
        ...
        saColorLabelVal.className = addon.tab.scratchClass(
            "color-picker_label-readout",
            {
        ...
        function updateHandle(e, keyPressed, originalPos) {
            let cx = Math.min(
                Math.max(
                    e.clientX - saColorPicker.getBoundingClientRect().x,
                    0,
                ),
                150,
            );
            let cy = Math.min(
                Math.max(
                    e.clientY - saColorPicker.getBoundingClientRect().y,
                    0,
                ),
                150,
            );
            if (keyPressed === "Shift") {
                if (Math.abs(cx - originalPos.x) > Math.abs(cy - originalPos.y))
                    cy = originalPos.y;
        ...
        function updateFinal(e, keyPressed, originalPos) {
            rateLimiter.limit(() => {
                ...
                if (keyPressed === "Shift") {
                    if (
                        Math.abs(ox - originalPos.x) >
                        Math.abs(oy - originalPos.y)
                    )
                        oy = originalPos.y;
        ...
        saColorPicker.addEventListener("pointerdown", (e) => {
            ...
            if (fillOrStroke === "fill")
                el = document.getElementsByClassName(
                    addon.tab.scratchClass("color-button_color-button-swatch"),
                )[0];
        ...
        prevEventHandler = ({ detail }) => {
            if (
                detail.action.type ===
                "scratch-paint/color-index/CHANGE_COLOR_INDEX"
            ) {
                setTimeout(() => {
                    updateColor();
                }, 100);
