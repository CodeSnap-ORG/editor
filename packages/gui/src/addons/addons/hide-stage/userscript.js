export default async function ({ addon, console, msg }) {
    ...
    function hideStage() {
        ...
        if (fullStageButton)
            fullStageButton.setAttribute("aria-pressed", false);
    ...
    function unhideStage(e) {
        ...
        if (e) {
            ...
            const selectedStageSize =
                addon.tab.redux.state.scratchGui.stageSize.stageSize;
            if (smallStageButton)
                smallStageButton.setAttribute(
                    "aria-pressed",
                    selectedStageSize === "small",
                );
            if (largeStageButton)
                largeStageButton.setAttribute(
                    "aria-pressed",
                    selectedStageSize === "large",
                );
            if (fullStageButton)
                fullStageButton.setAttribute(
                    "aria-pressed",
                    selectedStageSize === "full",
                );
        }
        window.dispatchEvent(new Event("resize")); // resizes the code area and paint editor canvas
    }
    ...
    while (true) {
        ...
        const stageButtons = Array.from(
            stageControls.querySelectorAll("button"),
        );
