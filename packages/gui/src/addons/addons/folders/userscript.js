...
export const addDefaultAssetFolderIfMissing = (asset) => {
    if (
        asset &&
        currentAssetFolder !== null &&
        typeof getFolderFromName(asset.name) !== "string"
    ) {
...
export default async function ({ addon, console, msg }) {
    ...
    const getSortableHOCFromElement = (el) => {
        const nearestSpriteSelector = el.closest(
            "[class*='sprite-selector_sprite-selector']",
        );
        if (nearestSpriteSelector) {
            return nearestSpriteSelector[reactInternalKey].child.sibling.child
                .stateNode;
        }
        const nearestAssetPanelWrapper = el.closest(
            '[class*="asset-panel_wrapper"]',
        );
        if (nearestAssetPanelWrapper) {
            return nearestAssetPanelWrapper[reactInternalKey].child.child
                .stateNode;
        }
...
<svg width="100px" height="100px" viewBox="0 0 20 20" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
    ...
    const fixCostumeOrder = (target = vm.editingTarget) => {
        const { items, changed } = fixOrderOfItemsInFolders(
    ...
    const verifySortableHOC = (sortableHOCInstance) => {
        ...
        if (
            ...
            (typeof sortableHOCInstance.props.selectedId === "string" ||
                typeof sortableHOCInstance.props.selectedItemIndex ===
                    "number") &&
    ...
    const verifySpriteSelectorItem = (spriteSelectorItemInstance) => {
        ...
        if (
            ...
            typeof SpriteSelectorItem.prototype.handleDuplicate ===
                "function" &&
    ...
    const patchSortableHOC = (SortableHOC, type) => {
        ...
        const createFolderPreview = (items) => {
            ...
            for (
                let i = 0;
                i < Math.min(PREVIEW_POSITIONS.length, items.length);
                i++
            ) {
                const item = items[i];
                            ...
                            if (isSelected) {
                                if (isOpen) {
                                    result.selectedItemIndex =
                                        items.length + folderItems.length;
                                } else {
        ...
        const getSelectedItem = (sortable) => {
            if (type === TYPE_SPRITES) {
                const selectedItem = sortable.props.items.find(
                    (i) => i.id === sortable.props.selectedId,
                );
                return selectedItem;
            } else if (type === TYPE_ASSETS) {
                const selectedItem =
                    sortable.props.items[sortable.props.selectedItemIndex];
                return selectedItem;
            }
            return null;
        };
        ...
        SortableHOC.prototype.componentDidUpdate = function (
            prevProps,
            prevState,
        ) {
            const selectedItem = getSelectedItem(this);
            if (selectedItem) {
                const folder = getFolderFromName(selectedItem.name);
                const currentFolder = this.state.folders.includes(folder)
                    ? folder
                    : null;
                if (type === TYPE_SPRITES) {
                ...
                if (this.props.selectedId) {
                    ...
                    selectedItemChanged =
                        ...
                        this.props.items[this.props.selectedItemIndex].name !==
                            prevProps.items[prevProps.selectedItemIndex].name;
                }
                if (selectedItemChanged) {
                    if (!selectedItem.isStage) {
                        if (
                            typeof folder === "string" &&
                            !this.state.folders.includes(folder)
                        ) {
    ...
    const isFolderOpen = (component, folder) => {
        ...
        const folders =
            (sortableHOCInstance.state && sortableHOCInstance.state.folders) ||
            [];
    ...
    addon.tab.createEditorContextMenu((ctxType, ctx) => {
        if (
            ...
            return;
        const component =
            ctx.target[addon.tab.traps.getInternalKey(ctx.target)].return.return
                .return.stateNode;
                ...
                if (component.props.dragType === "SPRITE") {
                    for (const target of vm.runtime.targets) {
                        if (target.isOriginal) {
                            if (
                                getFolderFromName(target.getName()) ===
                                data.folder
                            ) {
                                vm.renameSprite(
                                    target.id,
                    ...
                    for (
                        let i = 0;
                        i < vm.editingTarget.sprite.costumes.length;
                        i++
                    ) {
                        ...
                        if (getFolderFromName(costume.name) === data.folder) {
                            vm.renameCostume(
                                i,
                                setFolderOfName(costume.name, newName),
                            );
                        }
                    }
                    fixCostumeOrder();
                    ...
                    for (
                        let i = 0;
                        i < vm.editingTarget.sprite.sounds.length;
                        i++
                    ) {
                        ...
                        if (getFolderFromName(sound.name) === data.folder) {
                            vm.renameSound(
                                i,
                                setFolderOfName(sound.name, newName),
                            );
                        }
                    }
                    fixSoundOrder();
            ...
            const setFolder = (folder) => {
                if (component.props.dragType === "SPRITE") {
                    ...
                    vm.renameSprite(
                        component.props.id,
                        ensureNotReserved(
                            setFolderOfName(target.getName(), folder),
                        ),
                    );
                    fixTargetOrder();
                    vm.emitWorkspaceUpdate();
                    ...
                    vm.renameCostume(
                        vm.editingTarget.sprite.costumes.indexOf(asset),
                        setFolderOfName(asset.name, folder),
                    );
                    fixCostumeOrder();
                } else if (component.props.dragType === "SOUND") {
                    const data = getItemData(component.props);
                    ...
                    vm.renameSound(
                        vm.editingTarget.sprite.sounds.indexOf(asset),
                        setFolderOfName(asset.name, folder),
                    );
                    fixSoundOrder();
                }
    ...
    const patchSpriteSelectorItem = (SpriteSelectorItem) => {
        ...
        const originalHandleDragEnd =
            SpriteSelectorItem.prototype.handleDragEnd;
        SpriteSelectorItem.prototype.handleDragEnd = function (...args) {
            const itemData = getItemData(this.props);
            if (itemData) {
                if (
                    typeof itemData.realIndex === "number" &&
                    this.props.dragging
                ) {
                    ...
                    if (originalIndex !== realIndex) {
                        const currentOffset =
                            addon.tab.redux.state.scratchGui.assetDrag
                                .currentOffset;
                        const sortableHOCInstance = getSortableHOCFromElement(
                            this.ref,
                        );
                        if (
                            currentOffset &&
                            sortableHOCInstance &&
                            sortableHOCInstance.getMouseOverIndex() === null
                        ) {
                ...
                if (itemData) {
                    if (typeof itemData.folder === "string") {
                        ...
                        setFolderOpen(
                            this,
                            itemData.folder,
                            !isFolderOpen(this, itemData.folder),
                        );
                        return;
                    }
                    if (
    ...
    const patchVM = () => {
            ...
            if (currentSpriteFolder !== null) {
                ...
                if (Array.isArray(targets) && !wholeProject) {
                    for (const target of targets) {
                        if (target.sprite) {
                            target.sprite.name = setFolderOfName(
                                target.sprite.name,
                                currentSpriteFolder,
                            );
                        }
        ...
        const abstractReorder = (
            ...
            itemIndex = clamp(
                itemIndex,
                zeroIndexed ? 0 : 1,
                zeroIndexed ? guiItems.length - 1 : guiItems.length,
            );
            newIndex = clamp(
                newIndex,
                zeroIndexed ? 0 : 1,
            ...
            const reorderingItems =
                typeof targetItemData.folder === "string"
                    ? targetItem.items
                    : [targetItem];
            const reorderingAssets = reorderingItems
                .map((i) => getVMItemFromGUIItem(i, assets))
                .filter((i) => i);
            ...
            if (newIndex === (zeroIndexed ? 0 : 1)) {
                ...
                newFolder =
                    typeof itemAtNewIndexData.inFolder === "string"
                        ? itemAtNewIndexData.inFolder
                        : null;
                ...
                if (newIndex < itemIndex) {
                    ...
                    //   *----^
                    item =
                        itemAtNewIndex.items[itemAtNewIndex.items.length - 1];
                    offset = 1;
                }
            ...
            if (
                typeof targetItemData.folder !== "string" &&
                targetItemData.inFolder !== newFolder
            ) {
        ...
        vm.constructor.prototype.reorderTarget = function (
            targetIndex,
            newIndex,
        ) {
        ...
        RenderedTarget.prototype.reorderCostume = function (
            costumeIndex,
            newIndex,
        ) {
            return abstractReorder(
                {
                    ...
                    rename: (item, name) => {
                        this.renameCostume(
                            this.sprite.costumes.indexOf(item),
                            name,
                        );
                    },
        ...
        RenderedTarget.prototype.reorderSound = function (
            soundIndex,
            newIndex,
        ) {
            return abstractReorder(
                {
                    ...
                    rename: (item, name) => {
                        this.renameSound(
                            this.sprite.sounds.indexOf(item),
                            name,
                        );
                    },
        ...
        vm.shareSoundToTarget = function (...args) {
            ...
            if (!target) {
                ...
                return Promise.reject(
                    new Error("Dropping sound into folder is not supported"),
                );
    ...
    const patchBackpack = (backpackInstance) => {
        ...
        Backpack.prototype.componentDidUpdate = function (
            prevProps,
            prevState,
        ) {
        ...
        Backpack.prototype.handleDrop = function (...args) {
            ...
            const folderItems =
                dragInfo &&
                dragInfo.payload &&
                dragInfo.payload.sa_folder_items;
            if (Array.isArray(folderItems)) {
                addon.tab
                    .confirm("", msg("confirm-backpack-folder"), {
                        useEditorClasses: true,
                    })
                    .then((result) => {
                        if (!result) return;
        ...
        backpackInstance.handleDrop =
            Backpack.prototype.handleDrop.bind(backpackInstance);
    };
    ...
    // Backpack
    {
        const clickListener = (e) => {
            ...
            setTimeout(() => {
                const backpackContainer = document.querySelector(
                ...
                const backpackInstance =
                    getBackpackFromElement(backpackContainer);
    ...
    // Sprite list
    {
        const spriteSelectorItemElement = await addon.tab.waitForElement(
        ...
        reactInternalKey = Object.keys(spriteSelectorItemElement).find((i) =>
            i.startsWith(REACT_INTERNAL_PREFIX),
        );
        const sortableHOCInstance = getSortableHOCFromElement(
            spriteSelectorItemElement,
        );
        const spriteSelectorItemInstance =
            spriteSelectorItemElement[reactInternalKey].child.child.child
                .stateNode;
    ...
    {
        const selectorListItem = await addon.tab.waitForElement(
