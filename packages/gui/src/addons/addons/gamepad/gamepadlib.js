...
const getMovementConfiguration = (usedKeys) => ({
    usesArrows:
        usedKeys.has("ArrowUp") ||
        usedKeys.has("ArrowDown") ||
        usedKeys.has("ArrowRight") ||
        usedKeys.has("ArrowLeft"),
    usesWASD:
        (usedKeys.has("w") && usedKeys.has("s")) ||
...
class GamepadData {
    ...
    resetMappings() {
        ...
        this.buttonMappings = this.getDefaultButtonMappings().map(
            transformAndCopyMapping,
        );
        this.axesMappings = this.getDefaultAxisMappings().map(
            transformAndCopyMapping,
        );
    }

    clearMappings() {
        this.buttonMappings = createEmptyMappingList(
    ...
    getDefaultButtonMappings() {
        ...
        if (this.hints.importedSettings) {
            ...
            const possibleActionKeys = [
                ...
                "x",
                "c",
                ...Array.from(usedKeys).filter(
                    (i) => i.length === 1 && !possiblePauseKeys.includes(i),
                ),
            ];
...
class GamepadLib extends EventTarget {
    ...
    handleConnect(e) {
        ...
        this.dispatchEvent(
            new CustomEvent("gamepadconnected", { detail: gamepadData }),
        );
    }
    ...
    handleDisconnect(e) {
        ...
        this.dispatchEvent(
            new CustomEvent("gamepaddisconnected", { detail: gamepadData }),
        );
        if (this.gamepads.size === 0) {
    ...
    dispatchMouse(button, down) {
        if (down) {
            this.dispatchEvent(
                new CustomEvent("mousedown", { detail: button }),
            );
        } else {
...
class GamepadEditor extends EventTarget {
    constructor(gamepadLib) {
        ...
        this.gamepadLib.addEventListener(
            "gamepadconnected",
            this.onGamepadsChange,
        );
        this.gamepadLib.addEventListener(
            "gamepaddisconnected",
            this.onGamepadsChange,
    ...
    createAxisMapping(mappingList, index) {
        ...
        const updateDropdownValue = () => {
            if (
                mappingList[index].type === "key" ||
                mappingList[index].type === "mousedown"
            ) {
                if (
                    ...
                    mappingList[index].high ===
                        defaultAxesMappings.wasd[0].high &&
                    mappingList[index].low ===
                        defaultAxesMappings.wasd[0].low &&
                    mappingList[index + 1].high ===
                        defaultAxesMappings.wasd[1].high &&
                    mappingList[index + 1].low ===
                        defaultAxesMappings.wasd[1].low
                ) {
                    selector.value = "wasd";
                } else if (
                    mappingList[index].high ===
                        defaultAxesMappings.arrows[0].high &&
                    mappingList[index].low ===
                        defaultAxesMappings.arrows[0].low &&
                    mappingList[index + 1].high ===
                        defaultAxesMappings.arrows[1].high &&
                    mappingList[index + 1].low ===
                        defaultAxesMappings.arrows[1].low
                ) {
        ...
        selector.addEventListener("change", () => {
            if (selector.value === "custom") {
                ...
                if (mappingList[index].type !== "key") {
                    ...
                    mappingList[index + 1] = transformAndCopyMapping(
                ...
                mappingList[index] = transformAndCopyMapping(
                    defaultAxesMappings.wasd[0],
                );
                mappingList[index + 1] = transformAndCopyMapping(
                    defaultAxesMappings.wasd[1],
                );
                ...
                mappingList[index + 1] =
                    transformAndCopyMapping(emptyMapping());
            }
            updateOverlay();
            this.changed();
        });
    ...
    updateContent() {
        ...
        for (let i = 0; i < axesMappings.length; i += 2) {
            ...
            const { circleOverlay, selector } = this.createAxisMapping(
                axesMappings,
                i,
            );
    ...
    update(gamepads) {
        ...
        const gamepad = Array.from(gamepads).find(
            (i) => i && getGamepadId(i) === this.selector.value,
        );
        if (!gamepad) {
            return;
        }
    ...
    export() {
        ...
        return {
            ...
            buttons: gamepadData.buttonMappings.map(
                prepareButtonMappingForExport,
            ),
        };
    }
