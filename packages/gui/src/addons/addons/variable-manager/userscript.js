export default async function ({ addon, console, msg }) {
    ...
    manager.classList.add(
        addon.tab.scratchClass("asset-panel_wrapper"),
        "sa-var-manager",
    );
    ...
    varTabIcon.src =
        addon.self.getResource("/icon.svg") /* rewritten by pull.js */;
    ...
    function updateHeadingVisibility() {
        ...
        let filteredLocals = localVariables.filter(
            (v) => v.row.style.display !== "none",
        );
        let filteredGlobals = globalVariables.filter(
            (v) => v.row.style.display !== "none",
        );
        localHeading.style.display = filteredLocals.length === 0 ? "none" : "";
        globalHeading.style.display =
            filteredGlobals.length === 0 ? "none" : "";
    }
        ...
        buildDOM() {
            ...
            const onLabelOut = (e) => {
                ...
                if (this.target.isStage) {
                    ...
                    const existingNames = vm.runtime.getAllVarNamesOfType(
                        this.scratchVariable.type,
                    );
                    ...
                    nameAlreadyUsed = !!workspace.getVariable(
                        newName,
                        this.scratchVariable.type,
                    );
                }
    ...
    addon.tab.redux.addEventListener("statechanged", ({ detail }) => {
        if (detail.action.type === "scratch-gui/navigation/ACTIVATE_TAB") {
            ...
            if (
                !detail.action.isPlayerOnly &&
                addon.tab.redux.state.scratchGui.editorTab.activeTabIndex === 3
            ) {
