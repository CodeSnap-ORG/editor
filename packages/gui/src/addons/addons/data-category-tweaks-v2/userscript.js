export default async function ({ addon, console, msg, safeMsg }) {
    ...
    const separateVariablesByType = (toolboxXML) => {
        const listButtonIndex = toolboxXML.findIndex(
            (i) =>
                i.getAttribute("callbackkey") === "CREATE_LIST" ||
                i.getAttribute("type") === "data_addtolist",
        );
        return {
    ...
    const separateLocalVariables = (workspace, toolboxXML) => {
        ...
        return separateVariablesByScope(variables).concat(
            separateVariablesByScope(lists),
        );
    };
    ...
    const moveReportersDown = (toolboxXML) => {
        ...
        const moveReportersToEnd = (xml) => {
            ...
            for (const blockXML of xml) {
                if (
                    blockXML.hasAttribute("id") ||
                    blockXML.tagName === "BUTTON"
                ) {
            ...
            if (everythingElse.length) {
                everythingElse[everythingElse.length - 1].setAttribute(
                    "gap",
                    BIG_GAP,
                );
            }
    ...
    const variableCategoryCallback = (workspace) => {
        ...
        if (
            !addon.self.disabled &&
            addon.settings.get("separateLocalVariables")
        ) {
    ...
    addon.settings.addEventListener("change", (e) => {
        ...
        if (
            addon.settings.get("separateListCategory") !==
            hasSeparateListCategory
        ) {
            if (vm.editingTarget) {
                vm.emitWorkspaceUpdate();
            }
    ...
    const dynamicEnableOrDisable = () => {
        ...
        if (
            addon.settings.get("separateLocalVariables") ||
            addon.settings.get("moveReportersDown")
        ) {
            const workspace = Blockly.getMainWorkspace();
            if (workspace) {
                workspace.refreshToolboxSelection_();
