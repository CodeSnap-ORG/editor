export default async function ({ addon, console, msg }) {
    ...
    const ADDON_ITEMS = {
        ...
        createLocalVariable: {
            ...
            createVariable: (workspace, name) =>
                workspace.createVariable(name, "", null, true),
        },
        createGlobalList: {
        ...
        createLocalList: {
            ...
            createVariable: (workspace, name) =>
                workspace.createVariable(name, "list", null, true),
        },
        createBroadcast: {
            enabled: (name) =>
                canUseAsGlobalVariableName(name, "broadcast_msg"),
            createVariable: (workspace, name) =>
                workspace.createVariable(name, "broadcast_msg"),
        },
    };
    ...
    Blockly.DropDownDiv.show = function (...args) {
        ...
        blocklyDropdownMenu.insertBefore(
            searchBar,
            blocklyDropdownMenu.firstChild,
        );
        ...
        blocklyDropDownContent.style.width = getComputedStyle(
            blocklyDropDownContent,
        ).width;
        blocklyDropDownContent.style.height = getComputedStyle(
            blocklyDropDownContent,
        ).height;
    ...
    const oldFieldDropdownGetOptions =
    ...
    const oldFieldVariableOnItemSelected =
    ...
    Blockly.FieldVariable.prototype.onItemSelected = function (menu, menuItem) {
        ...
        if (
            ...
            if (Object.prototype.hasOwnProperty.call(ADDON_ITEMS, optionId)) {
                ...
                const variable = addonItem.createVariable(
                    workspace,
    ...
    function selectItem(item, click) {
        ...
        item.dispatchEvent(
            new MouseEvent("mousedown", { relatedTarget: item, bubbles: true }),
        );
        if (click)
            item.dispatchEvent(
    ...
    function performSearch() {
        ...
        const rank = (item, index) => {
            ...
            if (SCRATCH_ITEMS_TO_HIDE.includes(optionId)) {
                ...
                Object.prototype.hasOwnProperty.call(ADDON_ITEMS, optionId)
            ) {
                if (!query) {
                    return -1;
                }
                ...
                if (addonInfo.enabled(rawQuery)) {
                    item.element.lastChild.lastChild.textContent =
                        getMenuItemMessage(optionId)[0];
                    return 0;
                }
                return -1;
            }
    ...
    function updateSearch() {
        ...
        let needToUpdateDOM =
            previousSearchedItems.length !== searchedItems.length;
        if (!needToUpdateDOM) {
    ...
    function handleKeyDownEvent(event) {
        if (event.key === "Enter") {
            ...
            for (let i = 0; i < items.length; i++) {
                if (
                    items[i].element.classList.contains(
                        "goog-menuitem-highlight",
                    )
                ) {
                    selectedIndex = i;
