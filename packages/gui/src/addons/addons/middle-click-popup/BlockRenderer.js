...
const BlockShapes = {
    ...
    Round: {
        ...
        backgroundPath: (width) =>
            `m -12 -20 m 20 0 h ${width - 16} a 20 20 0 0 1 0 40 H 8 a 20 20 0 0 1 0 -40 z`,
    ...
    Boolean: {
        ...
        minWidth: 20,
        backgroundPath: (width) =>
            `m -20 -20 m 20 0 h ${width} l 20 20 l -20 20 H 0 l -20 -20 l 20 -20 z`,
    ...
    TextInput: {
        ...
        minWidth: 16,
        backgroundPath: (width) =>
            `m -12 -16 m 16 0 h ${width - 8} a 16 16 0 0 1 0 32 H 4 a 16 16 0 0 1 0 -32 z`,
    ...
    BooleanInput: {
        ...
        minWidth: 16,
        backgroundPath: (width) =>
            `m 0 -16 h ${width} l 16 16 l -16 16 h -16 l -16 -16 l 16 -16 z`,
...
function getShapeInfo(shape, isVertical) {
    ...
    if (shape === BlockShape.Stack)
        return isVertical ? BlockShapes.Stack : BlockShapes.HorizontalBlock;
    if (shape === BlockShape.Hat) return BlockShapes.Hat;
    if (shape === BlockShape.End)
        return isVertical ? BlockShapes.End : BlockShapes.HorizontalBlockEnd;
...
function createTextComponent(text, fillVar, container) {
    let textElement = container.appendChild(
...
function createBackedTextedComponent(
    text,
    container,
    shape,
    categoryClass,
    ...
    if (textElement.width < shape.minWidth) {
        textElement.dom.setAttribute(
            "x",
            (shape.minWidth - textElement.width) / 2,
        );
    }
...
export default function renderBlock(block, container) {
    var blockComponent = _renderBlock(
        block,
        container,
    ...
    blockComponent.dom.setAttribute(
        "transform",
        `translate(${blockComponent.padding}, 0)`,
    );
...
function _renderBlock(block, container, parentCategory, isVertical) {
    ...
    const categoryClass = COLOR_CLASSES.includes(category.name)
        ? "sa-block-color-" + category.name
        : null;
    ...
    for (let partIdx = 0; partIdx < block.typeInfo.parts.length; partIdx++) {
        ...
        if (partIdx === 0 || partIdx === block.typeInfo.parts.length - 1) {
            if (
                component.snuggleWith &&
                component.snuggleWith.indexOf(shape) !== -1
            ) {
                const positionDelta =
                    component.snugglePadding - component.padding;
