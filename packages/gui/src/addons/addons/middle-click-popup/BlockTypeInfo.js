...
export class BlockInput {
    ...
    constructor(type, inputIdx, fieldIdx) {
        if (this.constructor === BlockInput)
    ...
    getField(block) {
        if (this.fieldIdx === -1) {
            return this.getInput(block).connection.targetBlock().inputList[0]
                .fieldRow[0];
        } else {
...
export class BlockInputRound extends BlockInput {
    constructor(type, inputIdx, fieldIdx, defaultValue) {
        ...
        if (this.constructor === BlockInputRound)
            throw new Error("Abstract classes can't be instantiated.");
    ...
    setValue(block, value) {
        if (value instanceof BlockInstance) {
            ...
            if (!subblock.outputConnection)
                throw new Error(
                    'Cannot put block "' +
                        subblock.typeInfo.id +
                        '" into a round type input.',
                );
...
export class BlockInputNumber extends BlockInputRound {
    ...
    _toFieldValue(value) {
        ...
        if (type === "string") {
            ...
            if (isNaN(number))
                throw new Error(
                    'Cannot set numeric type input to string "' + value + '".',
                );
            return value;
        }
        throw new Error("Cannot set round type input to value of type " + type);
...
export class BlockInputBoolean extends BlockInput {
    ...
    setValue(block, value) {
        if (value instanceof BlockInstance) {
            const subblock = value.createWorkspaceForm();
            if (
                !subblock.outputConnection ||
                value.typeInfo.shape !== BlockShape.Boolean
            )
                throw new Error(
                    'Cannot put block "' +
                        value.typeInfo.id +
                        '" into a boolean type input.',
                );
            subblock.outputConnection.connect(this.getInput(block).connection);
        } else {
...
export class BlockInputColour extends BlockInput {
    ...
    setValue(block, value) {
        if (typeof value !== "string")
            throw new Error(
                "Cannot set color type input to value of type " + typeof type,
            );
...
export class BlockInputEnum extends BlockInput {
    ...
    constructor(options, inputIdx, fieldIdx, isRound) {
        ...
        for (let i = 0; i < options.length; i++) {
            if (
                typeof options[i][1] === "string" &&
                BlockInputEnum.INVALID_VALUES.indexOf(options[i][1]) === -1
            ) {
                this.values.push({
                    value: options[i][1],
    ...
    setValue(block, value) {
        if (this.isRound && value instanceof BlockInstance) {
            value
                .createWorkspaceForm()
                .outputConnection.connect(this.getInput(block).connection);
        } else {
            if (this.values.indexOf(value) === -1)
                throw new Error(
                    "Invalid enum value. Expected item from the values list.",
                );
...
export class BlockInputBlock extends BlockInput {
    ...
    setValue(block, value) {
        if (value instanceof BlockInstance) {
            const subblock = value.createWorkspaceForm();
            if (
                !subblock.previousConnection ||
                !value.typeInfo.shape.canStackUp
            )
                throw new Error(
                    'Cannot put block "' +
                        value.typeInfo.id +
                        '" into a block type input.',
                );
            subblock.previousConnection.connect(
                this.getInput(block).connection,
            );
        } else {
    ...
    createWorkspaceForm() {
        if (this.typeInfo.id === "control_stop") {
            this.typeInfo.domForm
                ...
                .setAttribute(
                    "hasnext",
                    "" + (this.inputs[0].value === "other scripts in sprite"),
                );
        }

        const block = this.typeInfo.Blockly.Xml.domToBlock(
            this.typeInfo.domForm,
        ...
        for (let i = 0; i < this.typeInfo.inputs.length; i++) {
            ...
            if (inputValue != null)
                this.typeInfo.inputs[i].setValue(block, inputValue);
        }
...
export class BlockTypeInfo {
    ...
    static getBlockCategory(block, vm) {
        ...
        if (block.type === "procedures_call") {
            if (vm.getAddonBlock(block.getProcCode()))
                name = "addon-custom-block";
            else name = "more";
    ...
    static _createBlocks(
            ...
            if (field.className_ === "blocklyText blocklyDropdownText") {
                ...
                if (!field.argType_) {
                    if (field.getText().trim().length !== 0)
                        parts.push(field.getText());
                } else if (field.argType_[0] === "colour") {
                    addInput(new BlockInputColour(inputIdx, fieldIdx));
                } else if (field.argType_[1] === "number") {
                    addInput(
                        new BlockInputNumber(inputIdx, fieldIdx, field.text_),
                    );
                } else {
                    addInput(
                        new BlockInputString(inputIdx, fieldIdx, field.text_),
                    );
                }
            }
        };
        ...
        for (
            let inputIdx = 0;
            inputIdx < workspaceForm.inputList?.length;
            inputIdx++
        ) {
            const input = workspaceForm.inputList[inputIdx];
            for (
                let fieldIdx = 0;
                fieldIdx < input.fieldRow.length;
                fieldIdx++
            ) {
            ...
            if (input.connection) {
                ...
                if (innerBlock) {
                    if (
                        innerBlock.inputList.length !== 1 ||
                        innerBlock.inputList[0].fieldRow.length !== 1
                    )
        ...
        if (workspaceForm.id === "of") {
            ...
            for (const targetInput of baseTargetInput.values) {
                ...
                if (isStage) {
                    ...
                    options = stageVariableOptions
                        .map((variable) => [variable, variable])
                        .concat(stageOptions);
                } else {
                    const sprite = vm.runtime.getSpriteTargetByName(
                        targetInput.value,
                    );
                    const spriteVariableOptions =
                        sprite.getAllVariableNamesInScopeByType("", true);
                    options = spriteVariableOptions
                        .map((variable) => [variable, variable])
                        .concat(spriteOptions);
                }

                const ofInputs = [];
                ofInputs[baseVarInputIdx] = new BlockInputEnum(
                    options,
                    baseVarInput.inputIdx,
                    baseVarInput.fieldIdx,
                    false,
                );
            ...
            const otherScriptsOptionIdx = oldInput.values.findIndex(
                (option) => option.string === "other scripts in sprite",
            );
            const otherScriptsOption = oldInput.values.splice(
                otherScriptsOptionIdx,
                1,
            )[0];
