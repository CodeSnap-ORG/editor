...
class TokenProvider {
    constructor(shouldCache) {
        if (this.constructor === TokenProvider)
...
class TokenTypeStringEnum extends TokenType {
        ...
        for (const value of values) {
            ...
            const parts = [];
            {
                ...
                for (let i = 0; i <= lower.length; i++) {
                    const char = lower[i];
                    if (
                        QueryInfo.IGNORABLE_CHARS.indexOf(char) !== -1 ||
                        !char
                    ) {
    ...
    *parseTokens(query, idx, depth) {
        for (let valueIdx = 0; valueIdx < this.values.length; valueIdx++) {
            ...
            let isDefiningFeature =
                !TokenTypeNumberLiteral.isValidNumber(substr);
...
class TokenTypeStringLiteral extends TokenType {
    ...
    *parseTokens(query, idx, depth) {
        ...
        for (let i = idx; i <= query.length; i++) {
            const isTerminator = TokenTypeStringLiteral.isTerminator(
                query.str[i],
            );
            const isIgnorable = QueryInfo.IGNORABLE_CHARS.includes(
                query.str[i],
            );
            if (
                (wasTerminator !== isTerminator || i == query.length) &&
                !wasIgnorable &&
                i !== idx &&
                i !== quoteEnd
            ) {
...
class TokenTypeNumberLiteral extends TokenType {
    ...
    *parseTokens(query, idx, depth) {
        for (let i = idx; i <= query.length; i++) {
            if (
                TokenTypeStringLiteral.isTerminator(query.str[i]) &&
                i !== idx
            ) {
                const value = query.str.substring(idx, i);
...
class TokenTypeColor extends TokenType {
    ...
    *parseTokens(query, idx, depth) {
        ...
        for (let i = 0; i < 6; i++) {
            if (
                TokenTypeColor.HEX_CHARS.indexOf(
                    query.lowercase[idx + i + 1],
                ) === -1
            )
                return;
        }
...
class TokenTypeBrackets extends TokenType {
    ...
    *parseTokens(query, idx, depth) {
    ...
    createText(token, query, endOnly) {
        ...
        text += token.innerToken.type.createText(
            token.innerToken,
            query,
            endOnly,
        );
        if (token.innerToken.end !== token.end)
            text += query.str.substring(token.innerToken.end, token.end - 1);
        text += ")";
        return text;
    }
...
class TokenTypeBlock extends TokenType {
                ...
                switch (blockPart.type) {
                    case BlockInputType.ENUM:
                        ...
                        if (blockPart.isRound) {
                            ...
                            enumGroup.pushProviders([
                                fullTokenProvider,
                                querier.tokenGroupRoundBlocks,
                            ]);
                ...
                if (typeof blockPart === "string") {
                    ...
                    for (const enumValue of blockPart.values) {
                        enumerateStringForms(
                            ...
                            [
                                ...strings,
                                ...enumValue.string.toLowerCase().split(" "),
                            ],
                            [...inputs, enumValue],
    ...
    *parseTokens(query, idx, depth) {
        ...
        for (const subtokens of this._parseSubtokens(
            query,
            idx,
            this.fullTokenProviders,
        )) {
        ...
        outer: for (const stringForm of this.stringForms) {
            ...
            while (true) {
                ...
                if (wordEnd === i) {
                    ...
                    for (
                        let formPartIdx = lastPartIdx + 1;
                        formPartIdx < stringForm.strings.length;
                        formPartIdx++
                    ) {
                    ...
                    hasDefiningFeature ||=
                        !TokenTypeNumberLiteral.isValidNumber(word);
    ...
    _createToken(query, idx, subtokenProviders, subtokens) {
        ...
        for (const subtoken of subtokens) {
            ...
            if (subtoken.isDefiningFeature && subtoken.start < query.length)
        ...
        return new Token(
            ...
            this,
            { subtokens },
            { precedence: this.block.precedence, isTruncated, isLegal },
        );
    }
    ...
    *_parseSubtokens(
        ...
        for (const token of tokenProvider.parseTokens(query, idx, depth + 1)) {
            ...
            if (this.block.precedence !== -1) {
                if (
                    ...
                    (tokenProviderIdx === 0 ||
                        ...
                        token.type.block.id !== "operator_not")
                )
                    continue;
                /**
                ...
                if (token.precedence === this.block.precedence) {
                    const inputIndex =
                        this.block.parts[tokenProviderIdx].inputIdx;
            ...
            if (
                !parseNextToken ||
                !token.isLegal ||
                tokenProviderIdx === subtokenProviders.length - 1
            ) {
                yield [token];
            } else {
    ...
    createBlockValue(token, query) {
        ...
        if (token.value.stringForm) {
            ...
            for (let i = 0; i < subtokens.length; i++) {
                ...
                if (typeof blockPart !== "string")
                    blockInputs.push(subtokens[i].createBlockValue(query));
            }
    ...
    createText(token, query, endOnly) {
            ...
            if (endOnly) {
                if (
                    ...
                    return (
                        ...
                        token.value.stringForm.strings[
                            token.value.lastPartIdx
                        ].substring(token.end - token.value.i) +
                        " " +
                        token.value.stringForm.strings
                            .slice(token.value.lastPartIdx + 1)
                            .join(" ")
                    );
                }
            }
        ...
        for (i = 0; i < subtokens.length; i++) {
            ...
            const subtokenText =
                subtoken.type.createText(subtoken, query, endOnly) ?? "";
            text += subtokenText;
            if (i !== subtokens.length - 1) {
                ...
                if (nextStart !== subtoken.end) {
                    ...
                    if (
                        ...
                        QueryInfo.IGNORABLE_CHARS.indexOf(
                            subtokenText.at(-1),
                        ) === -1
                    )
                        text += " ";
...
export class QueryResult {
    ...
    toText(endOnly) {
        return (
            this.token.type.createText(this.token, this.query, endOnly) ?? ""
        );
    }
            ...
            for (const part of block.typeInfo.parts) {
                ...
                if (typeof part === "string") {
                    ...
                    if (input instanceof BlockInstance) {
                        ...
                        part instanceof BlockInputString &&
                        input !== part.defaultValue
                    ) {
...
class QueryInfo {
    ...
    static skipUnignorable(str, idx) {
        while (
            QueryInfo.IGNORABLE_CHARS.indexOf(str[idx]) === -1 &&
            idx < str.length
        )
            ++idx;
        return idx;
    ...
    canCreateMoreNestedBlocks() {
        return (
            this.canCreateMoreTokens() &&
            this.resultCount < WorkspaceQuerier.MAX_RESULTS
        );
    }
}
...
export default class WorkspaceQuerier {
    ...
    queryWorkspace(queryStr) {
        if (!this.workspaceIndexed)
            throw new Error(
                "A workspace must be indexed before it can be queried!",
            );
        if (queryStr.trim().length === 0)
            return { results: [], illegalResult: null, limited: false };
        ...
        for (const option of this.tokenGroupBlocks.parseTokens(query, 0, 0)) {
            ...
            if (!limited && query.resultCount >= WorkspaceQuerier.MAX_RESULTS) {
                console.log(
                    "Warning: Workspace query exceeded maximum result count.",
                );
                limited = true;
            }
            ...
            if (!query.canCreateMoreTokens()) {
                console.log(
                    "Warning: Workspace query exceeded maximum token count.",
                );
                limited = true;
                break;
            }
        ...
        function searchToken(token) {
            ...
            if (subtokens)
                ...
                !(token.type instanceof TokenTypeStringLiteral) &&
                token.isProper &&
                !token.isTruncated
            )
        ...
        function checkValidity(token) {
            ...
            if (subtokens) {
                ...
                token.type instanceof TokenTypeStringLiteral &&
                !TokenTypeNumberLiteral.isValidNumber(token.value)
            ) {
                for (let i = token.start; i < token.end; i++)
     ...
     */
    _createTokenGroups() {
        this.tokenTypeStringLiteral = new TokenProviderSingleCache(
            new TokenTypeStringLiteral(),
        );
        this.tokenTypeNumberLiteral = new TokenProviderSingleCache(
        ...
        this.tokenGroupBoolean = new TokenProviderOptional(
            new TokenProviderGroup(),
        );
        ...
        this.tokenGroupNumber = new TokenProviderOptional(
            new TokenProviderGroup(),
        );
        ...
        this.tokenGroupString = new TokenProviderOptional(
            new TokenProviderGroup(),
        );
        ...
        this.tokenGroupStack = new TokenProviderOptional(
    ...
    _populateTokenGroups(blocks) {
        ...
        for (const block of blocks) {
            block.precedence = WorkspaceQuerier.ORDER_OF_OPERATIONS.indexOf(
                block.id,
            );
        }
        for (let i = blocks.length - 1; i >= 0; i--) {
            ...
            if (block.precedence !== -1) {
                const target =
                    blocks.length -
                    (WorkspaceQuerier.ORDER_OF_OPERATIONS.length -
                        (block.precedence - 1));
                if (i !== target) {
        ...
        for (const block of blocks) {
            ...
            switch (block.shape) {
                ...
                case BlockShape.Boolean:
                    this.tokenGroupBooleanBlocks.pushProviders([
                        blockTokenType,
                    ]);
                    break;
