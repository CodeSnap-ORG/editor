...
export default async function ({ addon, msg, console }) {
    ...
    const popupInputContainer = popupContainer.appendChild(
        document.createElement("div"),
    );
    popupInputContainer.classList.add(
        addon.tab.scratchClass("input_input-form"),
    );
    popupInputContainer.classList.add("sa-mcp-input-wrapper");

    const popupInputSuggestion = popupInputContainer.appendChild(
        document.createElement("input"),
    );
    popupInputSuggestion.classList.add("sa-mcp-input-suggestion");

    const popupInput = popupInputContainer.appendChild(
        document.createElement("input"),
    );
    popupInput.classList.add("sa-mcp-input");
    popupInput.setAttribute("autocomplete", "off");

    const popupPreviewContainer = popupContainer.appendChild(
        document.createElement("div"),
    );
    ...
    const popupPreviewScrollbarBackground =
        popupPreviewScrollbarSVG.appendChild(
    ...
    function openPopup() {
        ...
        blockTypes = BlockTypeInfo.getBlocks(
            Blockly,
            vm,
        ...
        blockTypes.sort((a, b) => {
            const prio = (block) =>
                ...
                block.id.startsWith("data_");
            return prio(b) - prio(a);
        });

        previewScale =
            window.innerWidth * 0.00005 +
            addon.settings.get("popup_scale") / 100;
        previewWidth =
            (window.innerWidth * addon.settings.get("popup_width")) / 100;
        previewMaxHeight =
    ...
    function updateInput() {
        ...
        if (popupInput.value.trim().length === 0) {
            ...
            for (const queryResult of queryResults) {
                blockList.push({
                    ...
                    autocompleteFactory: (endOnly) =>
                        queryResult.toText(endOnly),
                });
            }
        }

        // @ts-ignore Delete the old previews
        while (popupPreviewBlocks.firstChild)
            popupPreviewBlocks.removeChild(popupPreviewBlocks.lastChild);
        ...
        for (let resultIdx = 0; resultIdx < blockList.length; resultIdx++) {
            ...
            svgBackground.setAttribute(
                "transform",
                `translate(0, ${(y + height / 10) * previewScale})`,
            );
            ...
            const svgBlock = popupPreviewBlocks.appendChild(
        ...
        popupPreviewScrollbarBackground.setAttribute(
            "height",
            "" + previewHeight,
        );
    ...
    function updateSelection(newIdx) {
        ...
        if (oldSelection) {
            oldSelection.svgBackground.classList.remove(
                "sa-mcp-preview-block-bg-selection",
            );
            oldSelection.svgBlock.classList.remove(
        ...
        if (queryPreviews.length === 0 && queryIllegalResult) {
            popupInputSuggestion.value =
                ...
                queryIllegalResult
                    .toText(true)
                    .substring(popupInput.value.length);
            return;
        }

        const newSelection = queryPreviews[newIdx];
        if (newSelection && newSelection.autocompleteFactory) {
            newSelection.svgBackground.classList.add(
                "sa-mcp-preview-block-bg-selection",
            );
            newSelection.svgBlock.classList.add(
                "sa-mcp-preview-block-selection",
            );
            ...
            newSelection.svgBackground.scrollIntoView({
                block: "nearest",
                behavior:
                    Math.abs(newIdx - selectedPreviewIdx) > 1
                        ? "smooth"
                        : "auto",
            });
    ...
    function updateCursor() {
        ...
        const cursorPosRel =
            popupInput.value.length === 0
                ? 0
                : cursorPos / popupInput.value.length;

        let y = 0;
        for (
            let previewIdx = 0;
            previewIdx < queryPreviews.length;
            previewIdx++
        ) {
            ...
            var blockX = 5;
            if (
                blockX + preview.renderedBlock.width >
                previewWidth / previewScale
            )
                blockX +=
                    (previewWidth / previewScale -
                        blockX -
                        preview.renderedBlock.width) *
                    previewScale *
                    cursorPosRel;
            ...
            preview.svgBlock.setAttribute(
                "transform",
                `translate(${blockX}, ${blockY}) scale(${previewScale})`,
            );
    ...
    function updateScrollbar() {
        ...
        popupPreviewScrollbarHandle.setAttribute(
            "height",
            "" + scrollbarHeight,
        );
    ...
    function selectBlock() {
        ...
        try {
            ...
            let newBlockX = Math.floor(
                (mousePosition.x - (blockBounds.left + blockBounds.right) / 2) /
                    workspace.scale,
            );
            let newBlockY = Math.floor(
                (mousePosition.y - (blockBounds.top + blockBounds.bottom) / 2) /
                    workspace.scale,
            );
    ...
    Blockly.Gesture.prototype.doWorkspaceClick_ = function () {
        if (
            this.mostRecentEvent_.button === 1 ||
