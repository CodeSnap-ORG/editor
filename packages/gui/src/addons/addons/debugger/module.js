...
const compensateForTimePassedWhilePaused = (thread, pauseState) => {
    ...
    if (
        ...
        thread.compatibilityStackFrame.timer
    ) {
        thread.compatibilityStackFrame.timer.startTime +=
...
export const singleStep = () => {
    ...
    if (!steppingThread) {
        ...
        for (const target of vm.runtime.targets) {
            for (const soundId of Object.keys(
                target.sprite.soundBank.soundPlayers,
            )) {
                const soundPlayer =
                    target.sprite.soundBank.soundPlayers[soundId];
                if (soundPlayer.outputNode) {
                    ...
                    soundPlayer.outputNode.start(
                        ...
                        audioContext.currentTime -
                            soundPlayer.startingUntil +
                            vm.runtime.currentStepTime / 1000,
                    );
                    soundPlayer.startingUntil -=
                        vm.runtime.currentStepTime / 1000;
                }
        ...
        for (const thread of vm.runtime.threads) {
            if (pausedThreadState.has(thread)) {
                pausedThreadState.get(thread).time +=
                    vm.runtime.currentStepTime;
            }
        }
