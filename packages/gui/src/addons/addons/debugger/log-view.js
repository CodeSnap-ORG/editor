...
class LogView {
    ...
    hide() {
        this.visible = false;
        this.scrollTopWhenHidden = this.isScrolledToEnd
            ? "end"
            : this.scrollTop;
    }
    ...
    isInView(index, margin = 0) {
        ...
        return (
            topEdgeFromTop >= viewportStart + margin &&
            bottomEdgeFromTop <= viewportEnd - margin
        );
    }
    ...
    scrollTo(index) {
        ...
        const maximumScrollTop = Math.max(
            0,
            this.rows.length * this.rowHeight - this.height + 1,
        );
        ...
        this.scrollTop = Math.min(
            maximumScrollTop,
            index * this.rowHeight - this.rowHeight * 0.3,
        );
    ...
    _queueScrollToEnd() {
        if (
            this.visible &&
            this.canAutoScrollToEnd &&
            this.isScrolledToEnd &&
            !this.scrollToEndQueued
        ) {
            ...
            queueMicrotask(() => {
                ...
                if (this.isScrolledToEnd) {
                    const scrollEnd =
                        this.innerElement.scrollHeight -
                        this.innerElement.offsetHeight;
    ...
    updateContent() {
        ...
        const startIndex = clamp(
            scrollStartIndex - EXTRA_ROWS_BELOW,
            0,
            this.rows.length,
        );
        const endIndex = clamp(
            scrollStartIndex + rowsVisible + EXTRA_ROWS_ABOVE,
            0,
            this.rows.length,
        );
