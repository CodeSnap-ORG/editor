...
export default async function createThreadsTab({ debug, addon, console, msg }) {
    ...
    logView.generateRow = (row) => {
        ...
        indenter.style.setProperty(
            "--level",
            isHeader ? row.depth : row.depth + 1,
        );
        ...
        if (row.type === "compiled") {
            ...
            el.textContent =
                "Compiled threads can't be stepped and have no stack information.";
            root.appendChild(el);
        }

        if (row.targetId && row.blockId) {
            root.appendChild(
                debug.createBlockLink(
                    debug.getTargetInfoById(row.targetId),
                    row.blockId,
                ),
            );
        }
                ...
                blockInfo.running =
                    ...
                    (thread.isCompiled ||
                        (blockId === runningThread.peekStack() &&
                            stackFrameIdx ===
                                runningThread.stackFrames.length - 1));

                const result = [blockInfo];
                if (
                    stackFrame &&
                    stackFrame.executionContext &&
                    stackFrame.executionContext.startedThreads
                ) {
                    for (const thread of stackFrame.executionContext
                        .startedThreads) {
                        concatInPlace(
                            result,
                            createThreadInfo(thread, depth + 1),
                        );
                    }
                }
    ...
    onSingleStep(() => {
        ...
        queueMicrotask(() => {
            ...
            if (
                runningIndex !== -1 &&
                !logView.isInView(runningIndex, logView.rowHeight)
            ) {
                // Try to show the entire thread if we can fit it on screen
                let found = false;
                const maxScrollback = Math.floor(
                    logView.height / logView.rowHeight,
                );
                ...
                if (!found) {
                    ...
                    logView.scrollTo(
                        Math.max(0, runningIndex - maxScrollback + 5),
                    );
                }
            }
        });
