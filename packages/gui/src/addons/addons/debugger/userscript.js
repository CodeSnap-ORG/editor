...
export default async function ({ addon, console, msg }) {
    ...
    debuggerButton.className = addon.tab.scratchClass(
    ...
    debuggerButtonImage.className = addon.tab.scratchClass(
        "stage-header_stage-button-icon",
    );
    debuggerButtonImage.draggable = false;
    debuggerButtonImage.src =
        addon.self.getResource("/icons/debug.svg") /* rewritten by pull.js */;
    ...
    const setHasUnreadMessage = (unreadMessage) => {
        debuggerButtonContent.classList.toggle(
            "sa-debugger-unread",
            unreadMessage,
        );
    };
    ...
    const buttonContainerElement = Object.assign(
    ...
    compilerWarning.textContent =
        "The debugger works best when the compiler is disabled.";
    ...
    const moveInterface = (x, y) => {
        ...
        const clampedX = Math.max(
            0,
            Math.min(x - mouseOffsetX, width - interfaceContainer.offsetWidth),
        );
        const clampedY = Math.max(
            0,
            Math.min(
                y - mouseOffsetY,
                height - interfaceContainer.offsetHeight,
            ),
        );
    ...
    const createBlockPreview = (targetId, blockId) => {
        ...
        if (
            ...
            text = ScratchBlocks.ScratchMsgs.translate(
                "PROCEDURES_DEFINITION",
                "define %1",
            ).replace("%1", formatProcedureCode(proccode));
            category = "more";
        } else {
            ...
            category = jsonData?.extensions.includes("default_extension_colors")
                ? "pen"
                : jsonData.category;
            const isStatement =
                (jsonData.extensions &&
    ...
    vm.runtime.targets[0].constructor.prototype.makeClone = function (...args) {
        if (
            addon.settings.get("log_failed_clone_creation") &&
            !vm.runtime.clonesAvailable()
        ) {
            logsTab.addLog(
    ...
    vm.runtime.startHats = function (hat, optMatchFields, ...args) {
        if (
            addon.settings.get("log_broadcasts") &&
            hat === "event_whenbroadcastreceived"
        ) {
            logsTab.addLog(
                msg("log-msg-broadcasted", {
