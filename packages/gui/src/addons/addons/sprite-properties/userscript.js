export default async function ({ addon, console, msg }) {
    ...
    document.addEventListener("click", (e) => {
        if (
            e.detail === 2 &&
            e.target.closest('[class^="sprite-selector_scroll-wrapper_"]')
        ) {
    ...
    function togglePropertiesPanel() {
        const isCurrentlyOpen =
            document.body.classList.contains(SHOW_PROPS_CLASS);
    ...
    const isDirectionPopoverOpen = () =>
        document.querySelector(
            "body > div.Popover > div > div > [class*=direction-picker_button-row_]",
        );
    ...
    document.body.addEventListener(
        ...
        (e) => {
            if (
                e.target.matches('[class*="sprite-selector_sprite-selector_"]')
            ) {
    ...
    function applySettings() {
        const visibleByDefault =
            !addon.settings.get("autoCollapse") &&
            !addon.settings.get("hideByDefault");
    ...
    function createButton(className, iconPath, tooltip) {
        ...
        buttonIcon.setAttribute(
            "src",
            addon.self.getResource(iconPath),
    ...
    function injectInfoButton() {
        if (!infoButton) {
            infoButton = createButton(
                PROPS_INFO_BTN_CLASS,
                "/info.svg",
                msg("open-properties-panel-tooltip"),
            );
        }
        const selectedSprite = propertiesPanel.parentNode.querySelector(
    ...
    function injectCloseButton() {
        if (!closeButton) {
            closeButton = createButton(
                PROPS_CLOSE_BTN_CLASS,
                "/collapse.svg",
                msg("close-properties-panel-tooltip"),
            );
        }
    ...
    function updateWideLocaleMode() {
        ...
        const isWideLocale = !!propertiesPanel.querySelector(
            "[class^=label_input-group-column_]",
        );
        document.body.classList.toggle(
            "sa-sprite-properties-wide-locale",
            isWideLocale,
        );
    }
    ...
    while (true) {
        propertiesPanel = await addon.tab.waitForElement(
        ...
        const itemsWrapper = spriteSelector.querySelector(
