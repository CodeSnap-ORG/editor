import {
    modifiedCreateAllInputs,
    modifiedUpdateDeclarationProcCode,
} from "./modified-funcs.js";

export default async function ({ addon, console }) {
    function createArrow(direction, callback) {
        const path =
            direction === "left"
                ? "M 17 13 L 9 21 L 17 30"
                : "M 9 13 L 17 21 L 9 30";
    ...
    function addInputAfter(addInputFn, fnName) {
        return function () {
            ...
            const proc = sourceBlock
                ? sourceBlock.parentBlock_
                    ? sourceBlock.parentBlock_
                    : sourceBlock
                : this;
            ...
            if (fnName === "addLabelExternal") {
                ...
                if (lastInput.type === Blockly.DUMMY_INPUT) {
                    lastInput.fieldRow[0].setValue(
                        lastInput.fieldRow[0].getValue() + " %l",
                    );
                }
            }
            ...
            if (
                ...
                !addon.settings.get("InsertInputsAfter")
            )
                return addInputFn.call(this, ...arguments);

            let newPosition =
                getFieldInputNameAndIndex(selectedField, proc.inputList).index +
                1;

            addInputFn.call(proc, ...arguments);

            const lastInputName =
                proc.inputList[proc.inputList.length - 1].name;
    ...
    function shiftInput(procedureBlock, inputNameToShift, newPosition) {
        ...
        if (
            !(
                inputNameToShift &&
                newPosition >= 0 &&
                newPosition <= initialInputListLength
            )
        ) {
            return false;
        }

        const originalPosition = procedureBlock.inputList.findIndex(
            (input) => input.name === inputNameToShift,
        );
        const itemToMove = procedureBlock.inputList.splice(
            originalPosition,
            1,
        )[0];
    ...
    function shiftFieldCallback(sourceBlock, field, direction) {
        const proc = sourceBlock.parentBlock_
            ? sourceBlock.parentBlock_
            : sourceBlock;
        ...
        const { name, index } = getFieldInputNameAndIndex(
            field,
            proc.inputList,
        );
    ...
    function polluteProcedureDeclaration(
        ...
        for (const inputFn of [
            ...
            procedureDeclaration[inputFn] = addInputAfter(
                procedureDeclaration[inputFn],
                inputFn,
            );
        }
    }
    ...
    function depolluteProcedureDeclaration(procedureDeclaration) {
        ...
        for (const [inputFnName, originalFn] of Object.entries(
            originalAddFns,
        )) {
    ...
    function enableAddon() {
        ...
        if (addon.tab.redux.state.scratchGui.customProcedures.active) {
            polluteProcedureDeclaration(
                getExistingProceduresDeclarationBlock(),
                false,
            );
        }
        ...
        Blockly.FieldTextInputRemovable.prototype.showEditor_ = function () {
            ...
            createArrow("left", () =>
                shiftFieldCallback(this.sourceBlock_, this, "left"),
            );
            createArrow("right", () =>
                shiftFieldCallback(this.sourceBlock_, this, "right"),
            );
            selectedField = this;
        };
    }
    ...
    function disableAddon() {
        ...
        if (addon.tab.redux.state.scratchGui.customProcedures.active) {
            depolluteProcedureDeclaration(
                getExistingProceduresDeclarationBlock(),
            );
        }

        Blockly.FieldTextInputRemovable.prototype.showEditor_ =
            originalShowEditor;
        Blockly.WidgetDiv.DIV.querySelectorAll(
    ...
    const originalCreateAllInputs =
        Blockly.Blocks["procedures_declaration"].createAllInputs_;
    const originalUpdateDeclarationProcCode =
        Blockly.Blocks["procedures_declaration"].onChangeFn;
    const originalRemoveFieldCallback =
        Blockly.Blocks["procedures_declaration"].removeFieldCallback;
    const originalShowEditor =
        Blockly.FieldTextInputRemovable.prototype.showEditor_;
    let originalAddFns = {};
