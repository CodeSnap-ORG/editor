export default async function ({ addon }) {
    ...
    vm.constructor.prototype.addSprite = function (input) {
        ...
        const isEmpty =
            spriteObj.costumes?.[0]?.baseLayerMD5 ===
            "cd21514d0531fdffb22204e0ec5ed84a.svg";
        if (
            !addon.self.disabled &&
            (isEmpty || !spriteObj.tags || !addon.settings.get("library"))
        ) {
        ...
        return oldAddSprite.call(
            this,
            stringify ? JSON.stringify(spriteObj) : spriteObj,
        );
    };

    const registerDupPrototype = () => {
        const targetPrototype =
            vm.runtime.getTargetForStage().constructor.prototype;
            ...
            return oldDuplicate.call(this).then((newSprite) => {
                if (!addon.self.disabled) {
                    switch (addon.settings.get("duplicate")) {
                        case "custom":
                            newSprite.setXY(
                                addon.settings.get("x"),
                                addon.settings.get("y"),
                            );
                            break;
                        case "keep":
