export default async function ({ addon, console, msg }) {
    ...
    const paintEditorCanvasContainer = await addon.tab.waitForElement(
    ...
    const paperCanvas =
        paintEditorCanvasContainer[
            addon.tab.traps.getInternalKey(paintEditorCanvasContainer)
        ].child.child.child.stateNode;
    ...
    const getPaperCenter = () => {
        const backgroundGuideLayer = paper.project.layers.find(
            (i) => i.data.isBackgroundGuideLayer,
        );
    ...
    const injectPaperCanvas = () => {
        ...
        paperCanvas.recalibrateSize =
            PaperCanvas.prototype.recalibrateSize.bind(paperCanvas);
        paperCanvas.importImage =
            PaperCanvas.prototype.importImage.bind(paperCanvas);
    };
    ...
    const relayerOnionLayers = () => {
        ...
        if (settings.layering === "front") {
            ...
            const rasterLayer = project.layers.find(
                (i) => i.data.isRasterLayer,
            );
    ...
    const getTint = (red, green, blue, isBefore) => {
        ...
        return [
            referenceColor[0] * weighted,
            referenceColor[1] * weighted,
            referenceColor[2] * weighted,
        ];
    };
            ...
            if (item instanceof paper.Raster) {
                promises.push(
                    new Promise((resolve, reject) => {
                        ...
                        item.on("error", () =>
                            reject(
                                new Error("Raster inside SVG failed to load"),
                            ),
                        );
                    }),
                );
    ...
    const rasterizeVector = (root) => {
        ...
        raster.draw = function (...args) {
            const displayedSize =
                this.getView().getZoom() * window.devicePixelRatio;
            const newScale = Math.max(
                1,
                Math.min(maxScale, 2 ** Math.ceil(Math.log2(displayedSize))),
            );
        ...
        new Promise((resolve, reject) => {
            ...
            const handleLoad = (root) => {
                ...
                if (settings.mode === "tint") {
                    ...
                    recursePaperItem(root, (i) => {
                        if (i.strokeColor) {
                            i.strokeColor = getPaperColorTint(
                                i.strokeColor,
                                isBefore,
                            );
                        }
                        if (i.fillColor) {
                            ...
                            if (gradient) {
                                ...
                                for (const stop of gradient.stops) {
                                    stop.color = getPaperColorTint(
                                        stop.color,
                                        isBefore,
                                    );
                                }
                            } else {
                                i.fillColor = getPaperColorTint(
                                    i.fillColor,
                                    isBefore,
                                );
                            }
                ...
                if (
                    ...
                    if (
                        ...
                        rotationPoint = rotationPoint.subtract(
                            viewBox[0],
                            viewBox[1],
                        );
                    }
                    root.translate(
                        paperCenter.subtract(rotationPoint.multiply(2)),
                    );
                } else {
                    root.translate(
                        paperCenter.subtract(
                            root.bounds.width,
                            root.bounds.height,
                        ),
                    );
            ...
            paper.project.importSVG(asset, {
                ...
                onLoad: (root) => {
                    ...
                    resolve(
                        waitForAllRastersToLoad(root).then(() =>
                            handleLoad(root),
                        ),
                    );
                },
    ...
    const updateOnionLayers = async () => {
        ...
        const startIndex = Math.max(
            0,
            selectedCostumeIndex - settings.previous,
        );
        const endIndex = Math.min(
            costumes.length - 1,
            selectedCostumeIndex + settings.next,
        );
        ...
        try {
            ...
            for (let i = startIndex; i <= endIndex; i++) {
                ...
                const opacity =
                    (settings.opacity - settings.opacityStep * distance) / 100;
                if (opacity <= 0) {
                    continue;
                }
                layersToCreate.push({
            ...
            const onions = await Promise.all(
                layersToCreate.map(({ index, isBefore, opacity }) => {
                    ...
                    if (onionCostume.dataFormat === "svg") {
                        ...
                        onionCostume.dataFormat === "png" ||
                        onionCostume.dataFormat === "jpg"
                    ) {
                        return makeRasterOnion(
                            opacity,
                            onionCostume,
                            onionAsset,
                            isBefore,
                        ...
                        throw new Error(
                            `Unknown data format: ${onionCostume.dataFormat}`,
                        );
                    }
                }),
            );
    ...
    const settingsChanged = (onlyRelayerNeeded) => {
        if (
            (settings.previous === 0 && settings.next === 0) ||
            settings.opacity === 0
        ) {
            setEnabled(false);
            return;
    ...
    const controlsLoop = async () => {
        ...
        while (true) {
            const canvasControls = await addon.tab.waitForElement(
            ...
            const zoomControlsContainer = canvasControls.querySelector(
            ...
            if (!hasRunOnce) {
                ...
                const buttonClass =
                    zoomControlsContainer.firstChild.firstChild.className;
                const imageClass =
                    zoomControlsContainer.firstChild.firstChild.firstChild
                        .className;
