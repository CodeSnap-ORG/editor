...
export default async function ({ addon, msg, console }) {
    ...
    class FindBar {
        ...
        createDom(root) {
            ...
            this.findInput.className = addon.tab.scratchClass(
                "input_input-form",
                {
        ...
        tabChanged() {
            ...
            const tab =
                addon.tab.redux.state.scratchGui.editorTab.activeTabIndex;
        ...
        showDropDown(focusID, instanceBlock) {
            ...
            this.utils.offsetX =
                this.dropdownOut.getBoundingClientRect().width + 32;
        ...
        getScratchBlocks() {
            ...
            function getDescFromField(root) {
                ...
                for (const fieldRow of fields.fieldRow) {
                    desc = desc ? desc + " " : "";
                    if (
                        fieldRow instanceof Blockly.FieldImage &&
            ...
            for (const row of vars) {
                addBlock(
                    row.isLocal ? "var" : "VAR",
                    row.isLocal
                        ? msg("var-local", { name: row.name })
                        : msg("var-global", { name: row.name }),
                    row,
                );
            }
            ...
            for (const row of lists) {
                addBlock(
                    ...
                    row.isLocal
                        ? msg("list-local", { name: row.name })
                        : msg("list-global", { name: row.name }),
                    row,
                );
            }
            ...
            for (const event of events) {
                addBlock(
                    "receive",
                    msg("event", { name: event.eventName }),
                    event.block,
        ...
        getCallsToEvents() {
            ...
            for (const block of this.workspace.getAllBlocks()) {
                ...
                if (broadcastInput.type === "event_broadcast_menu") {
                    eventName =
                        broadcastInput.inputList[0].fieldRow[0].getText();
                } else {
        ...
        onItemClick(item, instanceBlock) {
            ...
            if (cls === "costume" || cls === "sound") {
                ...
                if (assetPanel) {
                    ...
                    let wrapper = assetPanel.closest(
                        "div[class*=gui_flex-wrapper]",
                    );
                    wrapper.scrollTop = 0;
                }
            } else if (
            ...
            for (const target of targets) {
                ...
                for (const id of Object.keys(blocks._blocks)) {
                    ...
                    if (
                        ...
                        const broadcastInputBlockId =
                            block.inputs.BROADCAST_INPUT.block;
                        const broadcastInputBlock =
                            blocks._blocks[broadcastInputBlockId];
                        if (broadcastInputBlock) {
                            let eventName;
                            if (
                                broadcastInputBlock.opcode ===
                                "event_broadcast_menu"
                            ) {
                                eventName =
                                    broadcastInputBlock.fields.BROADCAST_OPTION
                                        .value;
                            } else {
                                eventName = msg("complex-broadcast");
                            }
    ...
    class Carousel {
        ...
        createDom() {
            ...
            const leftControl = this.el.appendChild(
                document.createElement("span"),
            );
            ...
            const rightControl = this.el.appendChild(
                document.createElement("span"),
            );
        ...
        navSideways(e, dir) {
            if (this.blocks.length > 0) {
                this.idx =
                    (this.idx + dir + this.blocks.length) % this.blocks.length; // + length to fix negative modulo js issue.
                this.count.innerText =
                    this.idx + 1 + " / " + this.blocks.length;
                this.utils.scrollBlockIntoView(this.blocks[this.idx]);
            }
    ...
    while (true) {
        const root = await addon.tab.waitForElement(
            "ul[class*=gui_tab-list_]",
            {
                markAsSeen: true,
                reduxEvents: [
