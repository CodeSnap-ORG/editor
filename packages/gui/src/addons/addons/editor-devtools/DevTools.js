...
export default class DevTools {
    ...
    async init() {
        ...
        while (true) {
            const root = await this.addon.tab.waitForElement(
    ...
    async addContextMenus() {
        ...
        if (this.addon.settings.get("enableCleanUpPlus"))
            blockly.Msg.CLEAN_UP = this.m("clean-plus");
                ...
                if (
                    ...
                    items.push({
                        ...
                        callback: async () => {
                            ...
                            let varName = await window.prompt(
        ...
        for (const column of columns) {
            ...
            for (const block of column.blocks) {
                ...
                maxWidth = Math.max(
                    maxWidth,
                    Math.max(
                        heightWidth.width + extraWidth,
                        maxWidthWithComments,
                    ),
                );
            }
    ...
    getOrderedTopBlockColumns(separateOrphans) {
        ...
        for (const topBlock of topBlocks) {
            ...
            if (bestCol) {
                ...
                bestCol.x =
                    (bestCol.x * bestCol.count + position.x) / ++bestCol.count; // re-average the columns as more items get added...
    ...
    */
    ...
    eventKeyDown(e) {
        const switchCostume = (up) => {
            ...
            let selected = this.costTabBody.querySelector(
    ...
    initInner(root) {
        ...
        this.costTabBody = document.querySelector(
            "div[aria-labelledby=" + this.costTab.id + "]",
        );
        ...
        this.domHelpers.bindOnce(
            document,
            "mousemove",
            (...e) => this.eventMouseMove(...e),
            true,
        );
        this.domHelpers.bindOnce(
            document,
            "mousedown",
            (...e) => this.eventMouseDown(...e),
            true,
        ); // true to capture all mouse downs 'before' the dom events handle them
        this.domHelpers.bindOnce(
            document,
            "mouseup",
            (...e) => this.eventMouseUp(...e),
            true,
        );
    }
}
