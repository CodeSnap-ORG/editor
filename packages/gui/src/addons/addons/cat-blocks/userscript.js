...
export default async function ({ addon, console }) {
    ...
    Blockly.BlockSvg.prototype.renderCatFace_ = function () {
        ...
        var closedEye = Blockly.utils.createSvgElement(
            "path",
            {},
            this.svgFace_,
        );
        ...
        var closedEye2 = Blockly.utils.createSvgElement(
            "path",
            {},
            this.svgFace_,
        );
    ...
    Blockly.BlockSvg.prototype.initCatStuff = function () {
        ...
        this.catPath_.addEventListener("mouseenter", function (event) {
            ...
            if (event.target.svgFace.eye) {
                ...
                event.target.svgFace.closedEye.setAttribute(
                    "fill-opacity",
                    "0.6",
                );
                event.target.svgFace.closedEye2.setAttribute(
                    "fill-opacity",
                    "0.6",
                );
            }
            ...
            that.blinkFn = setTimeout(function () {
                if (event.target.svgFace.eye) {
                    event.target.svgFace.eye.setAttribute(
                        "fill-opacity",
                        "0.6",
                    );
                    event.target.svgFace.eye2.setAttribute(
                        "fill-opacity",
                        "0.6",
                    );
                    event.target.svgFace.closedEye.setAttribute(
    ...
    Blockly.BlockSvg.prototype.shouldWatchMouse = function () {
        ...
        var blockXOnScreen =
            xy.x > -MARGIN &&
            xy.x - MARGIN < screen.width / this.workspace.scale;
        var blockYOnScreen =
            xy.y > -MARGIN &&
            xy.y - MARGIN < screen.height / this.workspace.scale;
        return (
            this.startHat_ &&
