...
export const updateScaleTool = (paper, tool, vm) => {
    ...
    matchingSizeGuide.group.addChildren([
        matchingSizeGuide.endLeft,
        matchingSizeGuide.line,
        matchingSizeGuide.endRight,
    ]);
    ...
    const fixGuideSizes = () => {
        ...
        getLayer("isGuideLayer").addChildren([
            axisLineY,
            axisLineX,
            selectionSizeGuide.group,
            matchingSizeGuide.group,
        ]);
    };
    ...
    addons.paintSnap = function (event) {
        ...
        const doesSx =
            snapOn &&
            ((this.isCorner && !event.modifiers.shift) ||
                Math.abs(this.origSize.x) > 0.0000001);
        const doesSy =
            snapOn &&
            ((this.isCorner && !event.modifiers.shift) ||
                Math.abs(this.origSize.y) > 0.0000001);
            ...
            distance: (() => {
                switch (coord.type) {
                    case "width": {
                        return Math.abs(
                            Math.abs(size.x) -
                                Math.abs(coord.clamp.max - coord.clamp.min),
                        );
                    }
                    case "height": {
                        return Math.abs(
        ...
        const snapXCoords = createScalePoints(
            paper,
            lib,
            paintLayer.children,
            doesSx,
        ...
        const canSnapXCoords = Object.entries(snapXCoords)
            .map(format)
            .filter(canSnap)
            .sort(sortByPrioOrDist);

        const snapYCoords = createScalePoints(
            paper,
            lib,
            paintLayer.children,
            false,
        ...
        const canSnapYCoords = Object.entries(snapYCoords)
            .map(format)
            .filter(canSnap)
            .sort(sortByPrioOrDist);
        ...
        if (
            (Math.abs(oldSX) === Math.abs(sx) && closestSnapX) ||
            (Math.abs(oldSY) === Math.abs(sy) && closestSnapY)
        )
            fixGuideSizes();
        ...
        if (Math.abs(oldSX) === Math.abs(sx) && closestSnapX) {
            switch (closestSnapX.type) {
                case "width": {
                    ...
                    matchingSizeGuide.endLeft.firstSegment.point =
                        new paper.Point(closestSnapX.coord.clamp.min, matchy);
                    matchingSizeGuide.endLeft.lastSegment.point =
                        new paper.Point(
                            closestSnapX.coord.clamp.min,
                            matchy + 5 / paper.view.zoom,
                        );
                    matchingSizeGuide.endRight.firstSegment.point =
                        new paper.Point(closestSnapX.coord.clamp.max, matchy);
                    matchingSizeGuide.endRight.lastSegment.point =
                        new paper.Point(
                            closestSnapX.coord.clamp.max,
                            matchy + 5 / paper.view.zoom,
                        );
                    ...
                    selectionSizeGuide.endLeft.firstSegment.point =
                        new paper.Point(
                            this.itemGroup.bounds.left,
                            selectiony + 2 / paper.view.zoom,
                        );
                    selectionSizeGuide.endLeft.lastSegment.point =
                        new paper.Point(
                            this.itemGroup.bounds.left,
                            selectiony + 7 / paper.view.zoom,
                        );
                    selectionSizeGuide.endRight.firstSegment.point =
                        new paper.Point(
                            this.itemGroup.bounds.right,
                            selectiony + 2 / paper.view.zoom,
                        );
                    selectionSizeGuide.endRight.lastSegment.point =
                        new paper.Point(
                            this.itemGroup.bounds.right,
                            selectiony + 7 / paper.view.zoom,
                        );
                    selectionSizeGuide.line.firstSegment.point =
                        new paper.Point(
                            this.itemGroup.bounds.left,
                ...
                case "xcoord":
                case "itemSideVert": {
                    axisLineX.firstSegment.point = new paper.Point(
                        closestSnapX.coord.value,
                        bounds.top,
                    );
                    axisLineX.lastSegment.point = new paper.Point(
                        closestSnapX.coord.value,
                        bounds.bottom,
                    );
        ...
        if (Math.abs(oldSY) === Math.abs(sy) && closestSnapY) {
            switch (closestSnapY.type) {
                case "height": {
                    ...
                    matchingSizeGuide.endLeft.firstSegment.point =
                        new paper.Point(matchx, closestSnapY.coord.clamp.min);
                    matchingSizeGuide.endLeft.lastSegment.point =
                        new paper.Point(
                    ...
                    matchingSizeGuide.endRight.firstSegment.point =
                        new paper.Point(matchx, closestSnapY.coord.clamp.max);
                    matchingSizeGuide.endRight.lastSegment.point =
                        new paper.Point(
                            matchx - 5 / paper.view.zoom,
                            closestSnapY.coord.clamp.max,
                ...
                case "ycoord":
                case "itemSideHoriz": {
                    axisLineY.firstSegment.point = new paper.Point(
                        bounds.left,
                        closestSnapY.coord.value,
                    );
                    axisLineY.lastSegment.point = new paper.Point(
                        bounds.right,
                        closestSnapY.coord.value,
                    );
