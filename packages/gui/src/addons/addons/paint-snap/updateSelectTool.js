...
export const updateSelectTool = (paper, tool, vm) => {
    ...
    const fixGuideSizes = () => {
        ...
        guidePoint.addChildren([
            guidePointParts.shadow,
            guidePointParts.circle,
        ]);
    ...
    function onMouseDrag(event) {
        ...
        const selectionAnchor =
            getLayer("isGuideLayer").children.find(
                (c) => c.data.isSelectionBound,
            )?.selectionAnchor ?? {};

        const resetAnchorColor = () => {
            selectionAnchor.strokeColor = new paper.Color(
                0.30196078431372547,
                0.592156862745098,
                1,
            );
        ...
        if (!event.modifiers.shift && this.mode !== Modes.RESHAPE) {
            ...
            const snapPoints = createSnapPoints(
                paper,
                selectionBounds,
                lib,
                paintLayer.children,
            ...
            const configDefFn = (pointDef) => {
                ...
                if (
                    pointDef.type === "xcoord" ||
                    pointDef.type === "itemSideVert"
                )
                    return (point) =>
                        new paper.Point(
                            pointDef.value,
                            Math.min(
                                Math.max(point.y, pointDef.clamp.min),
                                pointDef.clamp.max,
                            ),
                        );
                if (
                    pointDef.type === "ycoord" ||
                    pointDef.type === "itemSideHoriz"
                )
                    return (point) =>
                        new paper.Point(
                            Math.min(
                                Math.max(point.x, pointDef.clamp.min),
                                pointDef.clamp.max,
                            ),
                            pointDef.value,
                        );
            ...
            const generateSnapPointsFor = (point) =>
                Object.fromEntries(
                    Object.entries(toPoints).map(([k, v]) => [
                        k,
                        { type: v.type, point: configDefFn(v)(point) },
                    ]),
                );

            const generatedSnapPoints = Object.entries(fromPoints).map(
                ([pointPos, point]) => ({
                    pointPos,
                    origPoint: point,
            ...
            const sortByPrioOrDist = (a, b) => {
                const prioDiff =
                    priority.indexOf(a.snapPointType) -
                    priority.indexOf(b.snapPointType);
            ...
            const closestSnapForEachPoint = generatedSnapPoints
                .map(({ point, origPoint, snapPoints }) => {
                    ...
                    const closestSnapPoint = snappablePoints.sort(
                        sortByPrioOrDist,
                    )[0] || {
                        pos: "",
                        distance: Infinity,
            ...
            const closestSnapPoint =
                closestSnapForEachPoint.sort(sortByPrioOrDist)[0];
            removeGuides();
            if (closestSnapPoint?.snapPoint) {
                fixGuideSizes();
                snapVector = closestSnapPoint.snapPoint.subtract(
                ...
                if (
                    closestSnapPoint.point.equals(this.selectionCenter) &&
                    closestSnapPoint.snapPointType === "point"
                ) {
                    selectionAnchor.fillColor = selectionAnchor.strokeColor =
                        new paper.Color(guideColor);
                } else {
                    resetAnchorColor();
                    switch (closestSnapPoint.snapPointType) {
                        ...
                        case "xcoord":
                        case "itemSideVert": {
                            guideLine.firstSegment.point = new paper.Point(
                                closestSnapPoint.snapPoint.x,
                                actionBounds.top,
                            );
                            guideLine.lastSegment.point = new paper.Point(
                                closestSnapPoint.snapPoint.x,
                                actionBounds.bottom,
                            );
