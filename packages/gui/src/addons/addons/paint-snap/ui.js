...
export function initUI({ addon, msg }) {
    ...
    document.body.addEventListener("click", (e) => {
        if (areSettingsOpen() && !e.target.matches(".sa-paint-snap-group *"))
    ...
    const setSettingsOpen = (open) => {
        ...
        if (open)
            ...
            Object.values(snapFrom).every((e) => !e) ||
            Object.values(snapTo).every((e) => !e)
        ) {
            toggle(false);
    ...
    const createSettingWithLabel = (settingId, settingElem) => {
        ...
        settingsOpenUpdaters.push(() => {
            const onBtn = settingElem.querySelector(
                `[aria-label="${msg("on")}"]`,
            );
            const offBtn = settingElem.querySelector(
                `[aria-label="${msg("off")}"]`,
            );
