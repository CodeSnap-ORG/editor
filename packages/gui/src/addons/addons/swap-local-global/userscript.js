export default async function ({ addon, msg, console }) {
    ...
    const getTargetName = (target) => {
        if (target.isStage) {
            ...
            return ScratchBlocks.ScratchMsgs.translate(
                "SENSING_OF_STAGE",
                "Stage",
            );
        }
    ...
    const getTargetsWithLocalVariableNamed = (name, type) =>
        vm.runtime.targets.filter(
            (target) =>
                target.isOriginal &&
                target.lookupVariableByNameAndType(name, type, true),
        );
    ...
    const customUndoVarDelete = function (forward) {
        ...
        if (forward) {
            _undoRedoPreserveStateCallback = beginPreservingState(
                workspace,
                this.varId,
            );
        ...
        return () => {
            ...
            if (oldMonitorState) {
                ...
                if (isLocal) {
                    ...
                    newMonitorState = newMonitorState.set(
                        "targetId",
                        target.id,
                    );
                    newMonitorState = newMonitorState.set(
                ...
                if (newVmVariable.name !== oldVmVariable.name) {
                    ...
                    if (block) {
                        newMonitorState = newMonitorState.set(
                            "params",
                            monitorBlocks._getBlockParams(block),
                        );
                    }
                }
    ...
    const convertVariable = (oldBlocklyVariable, newLocal, newCloud) => {
        ...
        if (isLocal !== newLocal) {
            if (newLocal) {
                ...
                const targets = getTargetsWithLocalVariableNamed(
                    name,
                    type,
                ).filter((target) => target !== editingTarget);
                if (targets.length > 0) {
                    alert(
    ...
    const canUserUseCloudVariables = () => {
        const blocksWrapper = document.querySelector(
            '[class^="gui_blocks-wrapper_"]',
        );
        let internalNode =
            blocksWrapper[addon.tab.traps.getInternalKey(blocksWrapper)];
        while (true) {
            if (!internalNode) {
                return false;
            }
    ...
    const addMoreOptionsToPrompt = (variable) => {
        ...
        const headerTitle = promptBody.parentElement.querySelector(
        ...
        const promptDisabledClass = addon.tab.scratchClass(
            "prompt_disabled-label",
        );

        const noLocalsInStageSection = document.createElement("div");
        noLocalsInStageSection.className = addon.tab.scratchClass(
            "prompt_info-message",
            "prompt_cloud-option",
            {
        ...
        noLocalsInStageSection.appendChild(
            Object.assign(document.createElement("span"), {
                textContent: addon.tab.scratchMessage(
        ...
        scopeSection.className = addon.tab.scratchClass(
            "prompt_options-row",
            "prompt_cloud-option",
        );
        ...
        const cloudCheckbox = createLabeledInput(
            addon.tab.scratchMessage("gui.gui.cloudVariableOption"),
            "checkbox",
        );
        ...
        const updateDisabledInputs = () => {
            ...
            forThisSpriteOnly.label.classList.toggle(
                promptDisabledClass,
                thisSpriteOnlyDisabled,
            );
    ...
    ScratchBlocks.Variables.renameVariable = function (
        ...
        const ret = originalRenameVariable.call(
            this,
            workspace,
            variable,
            (...args) => {
    ...
    addon.tab.createBlockContextMenu(
        (items, block) => {
            if (
                ...
                const variable = block.workspace.getVariableById(
                    block.getVars()[0],
                );
                if (variable) {
                    if (items.length > 0) {
                        if (
                            items[0].text ===
                            ScratchBlocks.ScratchMsgs.translate(
                                "RENAME_VARIABLE",
                            )
                        ) {
                            ...
                            items[0].text ===
                            ScratchBlocks.ScratchMsgs.translate("RENAME_LIST")
                        ) {
                    ...
                    items.push({
                        ...
                        separator: true,
                        text: msg(
                            `to-${variable.isLocal ? "global" : "local"}`,
                        ),
                        callback: () =>
                            convertVariable(
                                variable,
