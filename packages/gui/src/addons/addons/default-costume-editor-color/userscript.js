export default async function ({ addon, console, msg }) {
    ...
    const hexComponent = (str) =>
        Math.round(+str).toString(16).toUpperCase().padStart(2, "0");
    ...
    const getToolInfo = () =>
        TOOL_INFO[addon.tab.redux.state.scratchPaint.mode];
    ...
    const setDefaultColorsToSettings = () => {
        defaultFillColor = simpleHexColor(
            parseColor(addon.settings.get("fill")),
        );
        defaultStrokeColor = simpleHexColor(
            parseColor(addon.settings.get("stroke")),
        );
    ...
    const isValidColorToPersist = (color) =>
        color.primary !== null && color.primary !== MIXED;
    ...
    addon.tab.redux.addEventListener("statechanged", ({ detail }) => {
        ...
        if (!activatingTool && addon.settings.get("persistence")) {
            ...
            if (
                fillStyle.get(detail.prev) !== newFill &&
                isValidColorToPersist(newFill)
            ) {
                defaultFillColor = newFill;
            }
            const newStroke = strokeStyle.get();
            if (
                strokeStyle.get(detail.prev) !== newStroke &&
                isValidColorToPersist(newStroke)
            ) {
            ...
            queueMicrotask(() => {
                ...
                if (addon.settings.get("persistence")) {
                    ...
                    if (!toolInfo) {
                        console.warn(
                            "unknown tool",
                            addon.tab.redux.state.scratchPaint.mode,
                        );
                        return;
                    }
                    ...
                    if (
                        oldFillColor.primary === null ||
                        oldFillColor.primary === MIXED
                    ) {
                    ...
                    if (
                        oldStrokeColor.primary === null ||
                        oldStrokeColor.primary === MIXED
                    ) {
