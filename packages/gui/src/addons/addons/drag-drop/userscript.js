export default async function ({ addon, console }) {
    ...
    const globalHandleDragOver = (e) => {
        ...
        if (
            ...
            callback = (files) => {
                const hdFilter = addon.settings.get("use-hd-upload")
                    ? ""
                    : ":not(.sa-better-img-uploads-input)";
                const fileInput = el.querySelector(
                    'input[class*="action-menu_file-input"]' + hdFilter,
                );
            ...
            callback = (files) => {
                const contextMenuBefore = document.querySelector(
                ...
                // children of <body>)
                const contextMenuAfter = document.querySelector(
                    "body > .react-contextmenu.react-contextmenu--visible",
                );
                ...
                document.body.appendChild = (fileInput) => {
                    ...
                    if (fileInput instanceof HTMLInputElement) {
                        ...
                        console.error(
                            'File input was not immediately given to appendChild upon clicking "Import"!',
                        );
            ...
            callback = async (files) => {
                const text = (
                    await Promise.all(Array.from(files, (file) => file.text()))
                )
                    .join("")
