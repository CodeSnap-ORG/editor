export default async function ({ addon, console }) {
    ...
    const getButtonToClick = (mainButton) => {
        const assetPanelWrapper = mainButton.closest(
            "[class*=asset-panel_wrapper_]",
        );
        if (assetPanelWrapper) {
            if (
                addon.tab.redux.state.scratchGui.editorTab.activeTabIndex === 2
            ) {
                return soundMeta[getSetting("sound")] || soundMeta.library;
            } else {
                return (
                    costumeMeta[getSetting("costume")] || costumeMeta.library
                );
            }
        } else if (
            mainButton.closest('[class*="target-pane_stage-selector-wrapper"]')
        ) {
    ...
    document.body.addEventListener(
        ...
        (e) => {
            ...
            const mainButton = e.target.closest(
            ...
            e.stopPropagation();
            const moreButtonsElement = mainButton.parentElement.querySelector(
    ...
    document.body.addEventListener(
        ...
        (e) => {
            ...
            const tooltipElement = mainButton.parentElement.querySelector(
                ".__react_component_tooltip",
            );
            const { tooltip } = getButtonToClick(mainButton);
            const translatedTooltip =
                addon.tab.redux.state.locales.messages[tooltip];
            const needToFixTooltipText =
                translatedTooltip &&
