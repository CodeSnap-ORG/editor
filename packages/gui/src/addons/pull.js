...
const includeImportedLibraries = (contents) => {
    ...
    for (const match of matches) {
        ...
        const oldLibraryPath = pathUtil.resolve(
            __dirname,
            "ScratchAddons",
            "libraries",
            libraryFile,
        );
        const newLibraryPath = pathUtil.resolve(
            __dirname,
            "libraries",
            libraryFile,
        );
...
const detectUnimplementedAPIs = (addonId, contents) => {
    if (contents.includes("data-addon-id")) {
        console.warn(
            `Warning: ${addonId} seems to use data-addon-id. It should use [data-addons*=...] instead.`,
        );
    }
    ...
    if (contents.includes("addon.self.dir")) {
        ...
        console.warn(
            `Warning: ${addonId} contains un-rewritten addon.self.dir. It or this script should be modified so that it will be rewritten.`,
        );
    }
    ...
    if (contents.includes("addon.self.lib")) {
        // eslint-disable-next-line max-len
        console.warn(
            `Warning: ${addonId} contains un-rewritten addon.self.lib. It should use modern ES6 import statements.`,
        );
    }
};
...
const rewriteAssetImports = (contents) => {
    ...
    contents = contents.replace(
        /addon\.self\.(?:dir|lib) *\+ *([^;,\n]+)/g,
        (_fullText, name) =>
            `addon.self.getResource(${name}) /* rewritten by pull.js */`,
    );
...
const normalizeManifest = (id, manifest) => {
    const KEEP_TAGS = ["recommended", "theme", "beta", "danger"];
    ...
    const filterUserscripts = (scripts) =>
        scripts
            .filter(
                ({ matches }) =>
    ...
    if (manifest.credits) {
        for (const user of manifest.credits) {
            if (
                user.link &&
                !user.link.startsWith("https://scratch.mit.edu/")
            ) {
                console.warn(
                    `Warning: ${id} contains unsafe credit link: ${user.link}`,
                );
            }
...
const generateRuntimeEntry = (id, manifest, assets) => {
    ...
    for (const assetName of assets) {
        const importName = importSection.add(
            `!url-loader!./${assetName}`,
            "asset",
        );
...
const processAddon = (id, oldDirectory, newDirectory) => {
    ...
    const assets = files.filter((file) =>
        ASSET_EXTENSIONS.some((extension) => file.endsWith(extension)),
    );
    ...
    for (const file of files) {
        ...
        if (file === "addon.json") {
            ...
            const settingsEntryPath = pathUtil.join(
                newDirectory,
                "_manifest_entry.js",
            );
            fs.writeFileSync(
                settingsEntryPath,
                generateManifestEntry(id, parsedManifest),
            );

            const runtimeEntryPath = pathUtil.join(
                newDirectory,
                "_runtime_entry.js",
            );
            fs.writeFileSync(
                runtimeEntryPath,
                generateRuntimeEntry(id, parsedManifest, assets),
            );
            continue;
        }
...
const generateL10nSettingsEntries = (locales) =>
    generateEntries(
        locales.filter((i) => i !== "en"),
        (locale) => ({
...
for (const addon of addons) {
    const oldDirectory = pathUtil.resolve(
        __dirname,
        "ScratchAddons",
        "addons",
        addon,
...
const l10nFiles = fs.readdirSync(
    pathUtil.resolve(__dirname, "ScratchAddons", "addons-l10n"),
);
const languages = [];
...
for (const file of l10nFiles) {
    ...
    const runtimePath = pathUtil.resolve(
        __dirname,
        "addons-l10n",
        `${fixedName}.json`,
    );
    const settingsPath = pathUtil.resolve(
        __dirname,
        "addons-l10n-settings",
        `${fixedName}.json`,
    );
    const { settings, runtime, upstreamMessageIds } =
        parseMessageDirectory(oldDirectory);
...
for (const id of SKIP_MESSAGES) {
    if (!allUpstreamMessageIds.has(id)) {
        console.warn(
            `Warning: Translation ${id} is in SKIP_MESSAGES but does not exist`,
        );
    }
}

fs.writeFileSync(
    pathUtil.resolve(generatedPath, "l10n-entries.js"),
    generateL10nEntries(languages),
);
fs.writeFileSync(
    pathUtil.resolve(generatedPath, "l10n-settings-entries.js"),
    generateL10nSettingsEntries(languages),
);
fs.writeFileSync(
    pathUtil.resolve(generatedPath, "addon-entries.js"),
    generateRuntimeEntries(languages),
);
fs.writeFileSync(
    pathUtil.resolve(generatedPath, "addon-manifests.js"),
    generateManifestEntries(languages),
);
