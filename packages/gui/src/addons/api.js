        ...
        .filter(
            (styleSheet) =>
                !(
                    ...
                    (styleSheet.ownerNode.textContent.includes(
                        "input_input-form",
                    ) ||
                        styleSheet.ownerNode.textContent.includes(
                            "label_input-group_",
...
const removeMutationObserverCallback = (callback) => {
    _mutationObserverCallbacks = _mutationObserverCallbacks.filter(
        (i) => i !== callback,
    );
};
...
const updateClasses = () => {
    ...
    document.body.classList.toggle(
        "sa-small-stage",
        isSmallStage && !isFullScreen && !isPlayerOnly,
    );
    document.body.classList.toggle(
        "sa-body-editor",
...
class Tab extends EventTargetShim {
    constructor(id) {
        ...
        this.traps = {
            ...
            getPaper: async () => {
                const modeSelector = await this.waitForElement(
                ...
                const reactInternalKey = Object.keys(modeSelector).find((key) =>
                    key.startsWith("__reactInternalInstance$"),
                );
    ...
    waitForElement(
        selector,
        { markAsSeen = false, condition, reduxCondition, reduxEvents } = {},
    ) {
        let externalEventSatisfied = true;
        const evaluateCondition = () => {
            if (!externalEventSatisfied) return false;
            if (condition && !condition()) return false;
            if (reduxCondition && !reduxCondition(reduxInstance.state))
                return false;
            return true;
        };
        ...
        return new Promise((resolve) => {
            const callback = () => {
                ...
                for (const element of elements) {
                    ...
                    if (markAsSeen) this._seenElements.add(element);
                    if (reduxListener) {
                        this.redux.removeEventListener(
                            "statechanged",
                            reduxListener,
                        );
                    }
    ...
    appendToSharedSpace({ space, element, order, scope }) {
        ...
        const SHARED_SPACES = {
            ...
            fullscreenStageHeader: {
                ...
                element: () =>
                    q("[class^='stage-header_fullscreen-buttons-row_']"),
                from: () => [],
                until: () => [
                    q("[class^='stage-header_fullscreen-buttons-row_']")
                        .lastChild,
            ...
            afterSoundTab: {
                ...
                from: () => [
                    q("[class^='react-tabs_react-tabs__tab-list']").children[2],
                ],
    ...
    addBlock(procedureCode, { args, displayName, callback }) {
        ...
        if (displayName) {
            ...
            if (!compareArrays(procCodeArguments, displayNameArguments)) {
                console.warn(
                    `displayName ${displayName} for ${procedureCode} has invalid arguments, ignoring it.`,
                );
            ...
            this.traps.getBlockly().then((ScratchBlocks) => {
                ...
                BlockSvg.prototype.updateColour = function (...args2) {
                    ...
                    if (
                        ...
                        const block =
                            this.procCode_ &&
                            vm.runtime.getAddonBlock(this.procCode_);
                        if (block) {
                            const theme =
                                reduxInstance.state.scratchGui.theme.theme;
                ...
                ScratchBlocks.Blocks.procedures_call.createAllInputs_ =
                    function (...args2) {
                        ...
                        if (block && block.displayName) {
                            ...
                            const ret = originalCreateAllInputs.call(
                                this,
                                ...args2,
                            );
    ...
    createBlockContextMenu(
        ...
        this.traps.getBlockly().then((ScratchBlocks) => {
            ...
            ScratchBlocks.ContextMenu.show = function (event, items, rtl) {
                ...
                const blocklyContextMenu =
                    ScratchBlocks.WidgetDiv.DIV.firstChild;
                items.forEach((item, i) => {
                    if (i !== 0 && item.separator) {
                        ...
                        itemElt.style.borderTop =
                            "1px solid var(--ui-black-transparent)";
                    }
                });
            };
        });
    ...
    scratchClass(...args) {
        ...
        for (const arg of args) {
            if (typeof arg === "string") {
                for (const scratchClass of scratchClasses) {
                    if (
                        scratchClass.startsWith(`${arg}_`) &&
                        scratchClass.length === arg.length + 6
                    ) {
        ...
        if (typeof options === "object") {
            const others = Array.isArray(options.others)
                ? options.others
                : [options.others];
    ...
    recolorable() {
        ...
        const updateRealSrc = () => {
            ...
            nativeSrcSetter.call(
                image,
                `data:image/svg+xml;,${encodeURIComponent(newSrc)}`,
            );
        };
...
class AddonRunner {
    ...
    _msg(key, vars, handler) {
        const namespacedKey = key.startsWith("/")
            ? key.substring(1)
            : `${this.id}/${key}`;
    ...
    evaluateCustomCssVariable(variable) {
        ...
        switch (variable.type) {
            case "alphaBlend": {
                const opaqueSource = this.evaluateCustomCssVariable(
                    variable.opaqueSource,
                );
                const transparentSource = this.evaluateCustomCssVariable(
                    variable.transparentSource,
                );
                return textColorHelpers.alphaBlend(
                    opaqueSource,
                    transparentSource,
                );
            }
            case "alphaThreshold": {
                ...
                const threshold =
                    this.evaluateCustomCssVariable(variable.threshold) || 0.5;
                if (alpha >= threshold) {
            ...
            case "map": {
                return variable.options[
                    this.evaluateCustomCssVariable(variable.source)
                ];
            }
            case "multiply": {
            ...
            case "ternary": {
                const condition = this.evaluateCustomCssVariable(
                    variable.source,
                );
                return this.evaluateCustomCssVariable(
            ...
            case "textColor": {
                ...
                const threshold = this.evaluateCustomCssVariable(
                    variable.threshold,
                );
    ...
    async run() {
        ...
        if (this.manifest.userstyles) {
            for (let i = 0; i < this.manifest.userstyles.length; i++) {
                ...
                for (const [moduleId, cssText] of this.resources[
                    userstyle.url
                ]) {
        ...
        const disabledStylesheet = conditionalStyles.create(
            `_disabled/${this.id}`,
            disabledCSS,
        );
        disabledStylesheet.addDependent(
            this.id,
            baseStylePrecedence,
            () => this.publicAPI.addon.self.disabled,
        );
