...
const SETTINGS_KEY = process.env.ampmod_is_canary
    ? "canary:addons"
    : "tw:addons";
const VERSION = 5;
...
const migrateSettings = (settings) => {
    ...
    if (oldVersion < 3) {
        ...
        if (
            hideFlyout &&
            hideFlyout.enabled &&
            typeof hideFlyout.toggled === "undefined"
        ) {
...
class SettingsStore extends EventTargetShim {
    ...
    setAddonSetting(addonId, settingId, value) {
        ...
        if (value === null) {
            ...
            if (settingObject.type === "boolean") {
                ...
                settingObject.type === "integer" ||
                settingObject.type === "positive_integer"
            ) {
                ...
                if (
                    !settingObject.potentialValues.some(
                        (potentialValue) => potentialValue.id === value,
                    )
                ) {
                    throw new Error("Setting value is invalid.");
                }
    ...
    evaluateCondition(addonId, condition) {
        ...
        if (condition.settings) {
            // settings is an AND
            for (const [settingName, expectedValue] of Object.entries(
                condition.settings,
            )) {
                ...
                if (
                    !expectedValues.includes(
                        this.getAddonSetting(addonId, settingName),
                    )
                ) {
                    return false;
                }
