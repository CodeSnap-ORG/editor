...
const onReactContextMenu = function (e) {
    ...
    if (false && !ctxMenu && ctxTarget.closest(".monitor-overlay")) {
        ...
        const mInternalId =
            ctxTarget[this.traps.getInternalKey(ctxTarget)]?.return?.stateNode
                ?.props?.id;
        if (!mInternalId) return;
        ctxMenu = Array.prototype.find.call(
            document.querySelectorAll("body > nav.react-contextmenu"),
            (candidate) =>
                candidate[this.traps.getInternalKey(candidate)]?.return
                    ?.stateNode?.props?.id === mInternalId,
        );
        if (!ctxMenu) return;
        const props =
            ctxTarget[this.traps.getInternalKey(ctxTarget)]?.return?.return
        ...
        ctxTarget[this.traps.getInternalKey(ctxTarget)]?.return?.return?.return
            ?.stateNode?.props?.dragType
    ) {
    ...
    for (const item of hasDynamicContextMenu
        ? contextMenus.flatMap((menu) =>
              typeof menu === "function" ? menu(type, ctx) : menu,
          )
        : contextMenus) {
        if (!item) continue;
        if (item.types && !item.types.some((itemType) => type === itemType))
            continue;
...
const initialize = (tab) => {
    ...
    document.body.addEventListener(
        "contextmenu",
        (e) => onReactContextMenu.call(tab, e),
        { capture: true },
    );
};
