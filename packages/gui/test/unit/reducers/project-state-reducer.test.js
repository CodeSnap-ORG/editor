...
test(
    "onLoadedProject(LOADING_VM_WITH_ID, false, false), with project id, " +
...
// showing the original project.
test(
    "onLoadedProject(LOADING_VM_WITH_ID, false, false), with no project id, " +
...
test(
    "calling setProjectId, using with same id as already showing, " +
        "results in state SHOWING_WITH_ID",
    () => {
        const initialState = {
            projectId: "100",
...
test(
    "calling setProjectId, using different id from project already showing, " +
...
test(
    "projectError when creating new while viewing project with id should " +
...
test(
    "projectError encountered while in state FETCHING_WITH_ID results in " +
        "ERROR state",
    () => {
        const initialState = {
            error: null,
