...
test("unknown opcode", (t) => {
    ...
    vm.loadProject(project).then(() => {
        ...
        const secondBlockInputId =
            blocks.getBlock(secondBlockId).inputs.CONDITION.block;
        const thirdBlockInputId =
            blocks.getBlock(thirdBlockId).inputs.VOLUME.block;
        const fourthBlockInputId =
            blocks.getBlock(fourthBlockId).inputs.SOUND_MENU.block;
