...
const fixture = fs.readFileSync(
    path.join(__dirname, "..", "fixtures", "tw-serialize-asset-order.sb3"),
);
...
test("serializeAssets serialization order", (t) => {
    ...
    vm.loadProject(fixture).then(() => {
        ...
        for (let i = 0; i < assets.length; i++) {
            ...
            if (i < 8) {
                t.ok(
                    assets[i].fileName.endsWith(".svg"),
                    `file ${i + 1} is costume`,
                );
            } else {
                t.ok(
                    assets[i].fileName.endsWith(".wav"),
                    `file ${i + 1} is sound`,
                );
            }
        }
...
test("exportSprite serialization order", (t) => {
    ...
    vm.attachStorage(makeTestStorage());
    vm.loadProject(fixture).then(() => {
        vm.exportSprite(vm.runtime.targets[1].id, "arraybuffer").then(
            (serialized) => {
                JSZip.loadAsync(serialized).then((zip) => {
                    const files = Object.keys(zip.files);
                    for (let i = 0; i < files.length; i++) {
