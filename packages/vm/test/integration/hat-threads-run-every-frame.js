...
compilerAndInterpreter(
    "edge activated hat thread runs once every frame",
    (t, co) => {
...
compilerAndInterpreter(
    "edge activated hat thread runs after being added to previously executed target",
    (t, co) => {
        const vm = new VirtualMachine();
        vm.setCompilerOptions(co);
        vm.attachStorage(makeTestStorage());
 ...
 */
compilerAndInterpreter(
    "edge activated hat should trigger for both sprites when sprite is duplicated",
    (t, co) => {
 ...
 */
compilerAndInterpreter(
    "edge activated hat should trigger for both sprites when sprite is cloned",
    (t, co) => {
 ...
 */
compilerAndInterpreter(
    "edge activated hat thread does not interrupt stack click thread",
    (t, co) => {
        const vm = new VirtualMachine();
        vm.setCompilerOptions(co);
        vm.attachStorage(makeTestStorage());
