...
class JSGenerator {
        ...
        switch (node.kind) {
            ...
            case "list.get": {
                ...
                if (environment.supportsNullishCoalescing) {
                    ...
                    if (
                        index instanceof ConstantInput &&
                        index.constantValue === "last"
                    ) {
                        return new TypedInput(
            ...
            case "op.equals": {
                ...
                if (
                    leftAlwaysNumber &&
                    left instanceof ConstantInput &&
                    isSafeConstantForEqualsOptimization(left)
                ) {
                    return new TypedInput(
                        `(${left.asNumber()} === ${right.asNumber()})`,
                        TYPE_BOOLEAN
                ...
                if (
                    ...
                    isSafeConstantForEqualsOptimization(right)
                ) {
                    return new TypedInput(
                        `(${left.asNumber()} === ${right.asNumber()})`,
                        TYPE_BOOLEAN
    ...
    descendStackedBlock(node) {
        switch (node.kind) {
            ...
            case "motion.setXY": {
                ...
                const x =
                    "x" in node
                        ? this.descendInput(node.x).asNumber()
                        : "target.x";
                const y =
                    "y" in node
                        ? this.descendInput(node.y).asNumber()
                        : "target.y";
    ...
    safeConstantInput(value) {
        const unsafe =
            typeof value === "string" &&
            this.namesOfCostumesAndSounds.has(value);
