...
const migrate = async () => {
  ...
  if (
    ...
    dialog.showMessageBoxSync({
      ...
      detail: translate("arm-translation.message").replace(
        "{APP_NAME}",
        APP_NAME,
      ),
      buttons: [
  ...
  if (
    ...
    ((process.platform === "win32" &&
      getElectronMajorVersion() === 22 &&
      getKernelMajorVersion() >= 10) ||
    ...
    const result = dialog.showMessageBoxSync({
      ...
      detail: translate("unnecessary-legacy.message").replace(
        "{APP_NAME}",
        APP_NAME,
      ),
      buttons: [
  ...
  if (
    ...
    if (settings.dataVersion !== MigrateWindow.LATEST_VERSION) {
      changes.push(
        `S ${settings.dataVersion} -> ${MigrateWindow.LATEST_VERSION}`,
      );
    }
