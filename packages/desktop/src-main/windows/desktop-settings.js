...
class DesktopSettingsWindow extends AbstractWindow {
  constructor() {
    ...
    this.window.setTitle(
      `${translate("desktop-settings.title")} - ${APP_NAME}`,
    );
    ...
    this.ipc.handle(
      ...
      async (event, hardwareAcceleration) => {
    ...
    this.ipc.handle(
      ...
      async (event, backgroundThrottling) => {
    ...
    this.ipc.handle(
      "set-exit-fullscreen-on-escape",
      async (event, exitFullscreenOnEscape) => {
