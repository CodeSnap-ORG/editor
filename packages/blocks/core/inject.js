...
Blockly.createDom_ = function (container, options) {
  ...
  */
  ...
  var stackGlowFilter = Blockly.utils.createSvgElement(
    "filter",
    {
  ...
  options.stackGlowBlur = Blockly.utils.createSvgElement(
    "feGaussianBlur",
    {
      in: "SourceGraphic",
  ...
  var replacementGlowFilter = Blockly.utils.createSvgElement(
    "filter",
    {
      ...
      if (!mainWorkspace.isDragging()) {
        ...
        if (
          ...
          for (var b = 0, block; (block = blocks[b]); b++) {
            ...
            var overflowLeft =
              MARGIN + edgeLeft - blockXY.x - (options.RTL ? 0 : blockHW.width);
            if (overflowLeft > 0) {
...
Blockly.init_ = function (mainWorkspace) {
  ...
  var workspaceResizeHandler = Blockly.bindEventWithChecks_(
    window,
    "resize",
    null,
    function () {
...
Blockly.updateToolbox = function (tree) {
  console.warn(
    "Deprecated call to Blockly.updateToolbox, " +
