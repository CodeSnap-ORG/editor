...
function test_noWorkspaceComments() {
  ...
  try {
    assertEquals(
      "Empty workspace: no comments (1).",
      0,
      workspace.getTopComments(true).length,
    );
    assertEquals(
      "Empty workspace: no comments (2).",
      0,
      workspace.getTopComments(false).length,
    );
    workspace.clear();
    assertEquals(
      "Empty workspace: no comments (3).",
      0,
      workspace.getTopComments(true).length,
    );
    assertEquals(
      "Empty workspace: no comments (4).",
      0,
      workspace.getTopComments(false).length,
    );
  } finally {
...
function test_oneWorkspaceComment() {
  ...
  try {
    ...
    assertEquals(
      "One comment on workspace (1).",
      1,
      workspace.getTopComments(true).length,
    );
    assertEquals(
      "One comment on workspace  (2).",
      1,
      workspace.getTopComments(false).length,
    );
    assertEquals(
      "Comment db contains this comment.",
      comment,
      workspace.commentDB_["comment id"],
    );
    workspace.clear();
    assertEquals(
      "Cleared workspace: no comments (3).",
      0,
      workspace.getTopComments(true).length,
    );
    assertEquals(
      "Cleared workspace: no comments (4).",
      0,
      workspace.getTopComments(false).length,
    );
    assertFalse(
      "Comment DB does not contain this comment.",
      "comment id" in workspace.commentDB_,
    );
  } finally {
    workspaceCommentTest_tearDown();
  }
...
function test_getWorkspaceCommentById() {
  ...
  try {
    ...
    assertEquals(
      ...
      comment,
      workspace.getCommentById("comment id"),
    );
    assertEquals(
      "No comment found.",
      null,
      workspace.getCommentById("not a comment"),
    );
    comment.dispose();
    assertEquals(
...
function test_wsCommentText() {
  ...
  try {
    ...
    assertEquals(
      "Workspace undo stack has one event",
      1,
    ...
    assertEquals(
      "Comment text has changed",
      "new comment text",
      comment.getText(),
    );
    assertEquals(
      "Workspace undo stack has two events",
      2,
      workspace.undoStack_.length,
    );
    comment.dispose();
...
function test_workspaceCommentMinimizedFromXml() {
  ...
  try {
    ...
    comment.dispose();
    assertEquals(
      "Comment is no longer on workspace",
      null,
      workspace.getCommentById("comment id"),
    );
    ...
    assertNotEquals(
      "Comment loaded from xml is on workspace",
      null,
      importedComment,
    );
    assertEquals(
      "Imported comment is minimized",
      true,
      importedComment.isMinimized(),
    );
  } finally {
    workspaceCommentTest_tearDown();
  }
