...
function test_tokenizeInterpolation() {
  ...
  assertArrayEquals(
    "String table reference, lowercase",
    ["test string"],
    tokens,
  );
  tokens = Blockly.utils.tokenizeInterpolation("%{BKY_STRING_REF}");
  assertArrayEquals(
    "String table reference, uppercase",
    ["test string"],
    tokens,
  );
  ...
  assertArrayEquals(
    "String table reference, with parameter",
    ["before ", 1, " after"],
    tokens,
  );
  ...
  assertArrayEquals(
    "String table reference, with subreference",
  ...
  assertArrayEquals(
    "Undefined string table reference",
    ["%{bky_undefined}"],
    tokens,
  );
  ...
  assertArrayEquals(
    "Invalid initial digit in string table reference",
    ["before %{1} after"],
    tokens,
  );
  ...
  assertArrayEquals(
    "Invalid character in string table reference: space",
    ["before %{two words} after"],
    tokens,
  );
  ...
  tokens = Blockly.utils.tokenizeInterpolation("before %{two-words} after");
  assertArrayEquals(
    "Invalid character in string table reference: dash",
    ["before %{two-words} after"],
    tokens,
  );
  ...
  tokens = Blockly.utils.tokenizeInterpolation("before %{two.words} after");
  assertArrayEquals(
    "Invalid character in string table reference: period",
    ["before %{two.words} after"],
    tokens,
  );
  ...
  assertArrayEquals(
    "Invalid character in string table reference: &",
    ["before %{ab&c} after"],
    tokens,
  );
