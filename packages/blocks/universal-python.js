...
function run(command, args) {
  ...
  subprocess.on("error", function (error) {
    console.log(
      "universal-python: Error starting python; please ensure you have python installed: " +
        error,
    );
    process.exit(1);
  });
  ...
  if (python3) {
    ...
    if (python) {
      ...
      if (python2) {
        ...
        console.log(
          "universal-python: Could not find python on your PATH; please ensure you have python installed.",
        );
        process.exit(1);
      }
    }
