...
const _parseTransform = function (domElement) {
    ...
    const string =
        ...
        domElement.attributes.transform &&
        domElement.attributes.transform.value;
...
const _calculateTransformedEllipse = function (
    ...
    const rotA =
        (cosT * cosT) / radiusX / radiusX + (sinT * sinT) / radiusY / radiusY;
    const rotB = sin2T / radiusX / radiusX - sin2T / radiusY / radiusY;
    const rotC =
        (sinT * sinT) / radiusX / radiusX + (cosT * cosT) / radiusY / radiusY;
    ...
    let temp =
        (A - 1 / newRadiusXOverDet / newRadiusXOverDet) /
    ...
    if (
        Math.abs(Math.sin(2 * newTheta2) - temp) <
...
const _transformPath = function (pathString, transform) {
    if (
        !transform ||
        Matrix.toString(transform) === Matrix.toString(Matrix.identity())
    )
        return pathString;
    // First split the path data into parts of command-coordinates pairs
    // Commands are any of these characters: mzlhvcsqta
    const parts =
        pathString && pathString.match(/[mlhvcsqtaz][^mlhvcsqtaz]*/gi);
    let coords;
    let relative = false;
    let previous;
    let control;
        ...
        switch (lower) {
            ...
            case "a":
                ...
                for (let j = 0; j < length; j += 7) {
                    ...
                    const newEllipse = _calculateTransformedEllipse(
                        rx,
                        ry,
                        rotation,
                        transform,
                    ...
                    if (newEllipse) {
                        ...
                        result +=
                            `A ${roundTo4Places(Math.abs(newEllipse.radiusX))} ` +
...
const _createGradient = function (gradientId, svgTag, bbox, matrix) {
    ...
    const getValue = function (
        ...
        if (/%\s*$/.test(value)) {
            const size = allowPercent
                ? 1
                : bbox[/x|^width/.test(name) ? "width" : "height"];
            return (res / 100) * size;
        }
        return res;
    };
    ...
    const scaleToBounds =
        getValue(newGradient, "gradientUnits", true) !== "userSpaceOnUse";
    let origin;
    let destination;
    let radius;
    let focal;
    if (radial) {
        ...
        if (origin.x === destination.x && origin.y === destination.y) {
            ...
            if (
                !stops.length ||
                !stops[stops.length - 1].attributes ||
    ...
    if (radial) {
        ...
        let gradientPerpendicular =
            Math.abs(origin.x - destination.x) < 1e-8
                ? add(origin, {
                      x: 1,
                      y:
        ...
        const gradientDirection =
            Math.abs(originToPerpendicular.x) < 1e-8
                ? {
                      x: 1,
        ...
        const projectionCoeff =
            dot(originToDestination, gradientDirection) /
            dot(gradientDirection, gradientDirection);
        ...
        if (_isContainerElement(element)) {
            ...
            if (element.attributes) {
                ...
                if (element.attributes.stroke)
                    stroke = element.attributes.stroke.value;
            }
            ...
            if (
                Matrix.toString(matrix) === Matrix.toString(Matrix.identity())
            ) {
                element.removeAttribute("transform");
            ...
            if (fillGradientId || strokeGradientId) {
                ...
                if (fillGradientId) {
                    const newFillRef = _createGradient(
                        fillGradientId,
                        svgTag,
                        bbox,
                        matrix,
                ...
                if (strokeGradientId) {
                    const newStrokeRef = _createGradient(
                        strokeGradientId,
                        svgTag,
                        bbox,
                        matrix,
