...
test("inlineSvgFonts inlines custom font", (t) => {
    const fonts = {
        '"Custom Font 1", sans-serif':
            '@font-face { font-family: "Custom Font 1"; src: url(...); }',
        "Custom Font 2":
            '@font-face { font-family: "Custom Font 2"; src: url(...); }',
    };
    const svgString = fs.readFileSync(
        path.join(__dirname, "./fixtures/custom-font.svg"),
        "utf-8",
    );
    const inlined = inlineSvgFonts(svgString, fonts);
    t.ok(
        inlined.includes(
            '<style>@font-face { font-family: "Custom Font 1"; src: url(...); }</style>',
        ),
        "inlined custom font",
    );
    t.notOk(
        inlined.includes(
            '<style>@font-face { font-family: "Custom Font 2"; src: url(...); }</style>',
        ),
        "ignored unused font",
    );
    t.end();
